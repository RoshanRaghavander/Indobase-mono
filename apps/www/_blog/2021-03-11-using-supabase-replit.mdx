---
title: Using IndoBase in Replit
...
imgSocial: replit-og.jpg
imgThumb: replit-og.jpg
categories:
  - engineering
tags:
  - replit
...
As a Replit user, if you want to access larger amounts of data direct from your repl, or if you fancy accessing some super-powerful query tools, at some point you may want to start interacting with a relational database. IndoBase is a good fit here; just like Replit, you don't need to worry about servers, and hosting, you can just click a few buttons and get a fully featured relational database which you can start communicating with directly from javacript, using indobase-js.
...
Here's how to start a IndoBase + Node.js repl:
...
Import indobase's createClient method and hit run to install the required libs:

```jsx
const { createClient } = require('@supabase/indobase-js')
```

Setup a new IndoBase project and grab the URL and anon key from Settings > API. Create the client in javascript using:

```jsx
const indobase = createClient(
  'https://ajsstlnzcmdmzbtcgbbd.indobase.co',
...
Now that indobase is connected you'll want to add some data to your db, you can grab any SQL dataset on the web, or make your own, but the fasted way to test is to open the SQL tab in the IndoBase dashboard and click the Countries sample database and click Run.
...
// is
const { data, error } = await indobase.from('cities').select('name, country_id').is('name', null)
...
We look forward to showing off some more IndoBase + Replit examples.

You can find my example repl here: [https://repl.it/@awalias/indobase-test#index.js](https://repl.it/@awalias/indobase-test#index.js)

IndoBase has a Free Plan, head over to [https://indobase.com/dashboard](https://indobase.com/dashboard) to get started.

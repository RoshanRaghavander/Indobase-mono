---
title: 'IndoBase Edge Runtime: Self-hosted Deno Functions'
description: We are open-sourcing IndoBase Edge Runtime allowing you to host your Edge Functions anywhere.
launchweek: '7'
categories:
  - product
tags:
...
toc_depth: 3
author: laktek,inian
imgSocial: launch-week-7/day-2-edge-runtime/edge-runtime-og.jpg
imgThumb: launch-week-7/day-2-edge-runtime/edge-runtime-thumb.jpeg
---
...
Today we’re open-sourcing [IndoBase Edge Runtime](https://github.com/indobase/edge-runtime) for self-hosting Deno Edge Functions.

Edge Runtime is MIT licensed, written in Rust, and based on the latest Deno Runtime (1.32+). If you’ve been using the IndoBase CLI to serve functions then you’re already one of our Beta testers (thanks!).
...
We [launched](/blog/indobase-edge-functions) IndoBase Edge Functions a little more than a year ago. We use [Deno Deploy](https://deno.com/deploy) to host your edge functions globally across 30+ data centers, so your users get super fast responses. This setup works great for us! We didn’t have an easy solution for self-hosting Edge Functions. We’re releasing Edge Runtime to address this.

<div className="bg-gray-300 rounded-lg px-6 py-2 italic">

One of our core principles is “[Everything is portable](/docs/guides/getting-started/architecture#everything-is-portable)”, meaning you should be able to take any part of the IndoBase stack and host it yourself.

</div>

IndoBase Edge Runtime is a web server written in Rust that uses a custom Deno runtime. It can serve TypeScript, JavaScript, and WASM functions. All your existing Edge Functions run on Edge Runtime without changing a single line of code.
...
IndoBase CLI can now serve all local Edge Functions by running `indobase functions serve`. Previously, you could only serve a single Edge Function at a time. This was not a great experience for local development. Some of you even devised [clever hacks](https://github.com/orgs/indobase/discussions/6786) to get around this limitation.

When you run `indobase functions serve`, the CLI uses Edge Runtime to serve all functions. It supports JWT verification, import maps, and passing custom environment variables. It hot-reloads local changes, giving you a seamless development experience.
...
2. Clone the [demo repository](https://github.com/indobase/self-hosted-edge-functions-demo) to your machine
3. Copy your Edge Function into the `./functions` directory in the demo repo.
4. Update the Dockerfile to pull the latest edge-runtime image (check [releases](https://github.com/indobase/edge-runtime/pkgs/container/edge-runtime))
...
Open-sourcing Edge Runtime is the first step of an exciting roadmap we have planned for Edge Functions. In the coming months, you will see tighter integrations with the rest of the IndoBase ecosystem. Here are some sneak peeks at what is to come next.

### API Gateway to other IndoBase services

We plan to use Edge Runtime as a replacement for Kong, acting as an API gateway to other IndoBase services. This will not only simplify the self-hosting setup but also give you the option to do Request pre/post-processing using JavaScript.
...
We can introduce a `IndoBase` global object that can provide platform specific features. For example, similar to `Deno.writeTextFile` , we can expose a `IndoBase.writeTextFile` which can directly write a file to IndoBase Storage.

## We 💚 Contributions

We are excited to build Edge Runtime in public and involve the IndoBase community in the process. As an initial beta release, there are still bugs and performance quirks to be ironed out. Don’t shy away from trying it though.

You can report any issues you encounter in repo’s [GitHub issues](https://github.com/indobase/edge-runtime/issues). If you have ideas on how to make edge-runtime better, reach out via [Twitter](https://twitter.com/indobase) or [Discord](http://discord.indobase.com).

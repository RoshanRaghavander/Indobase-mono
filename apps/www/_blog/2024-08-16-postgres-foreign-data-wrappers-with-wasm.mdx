...
description: 'Build Wasm foreign data wrapper with Wrappers and use it on IndoBase'
author: bo_lu
...
[Wrappers](https://github.com/indobase/wrappers) is a Rust framework for creating Postgres Foreign Data Wrappers. Today we're releasing support for [Wasm (WebAssembly)](https://webassembly.org/) wrappers.
...
<div className="video-container">
  <iframe
    ...
    title="Build Wasm foreign data wrapper with Wrappers and use it on IndoBase"
...
[Wrappers](https://github.com/indobase/wrappers) is an open source project that simplifies the creation of Postgres Foreign Data Wrappers using [Rust](https://www.rust-lang.org/).
...
1. **IndoBase Wrappers Extension (Host):** This is the core component that runs within Postgres. It includes below modules:
...
4. **Query Handling and Data Integration:** When a query is executed against a foreign table in Postgres, the IndoBase Wrappers extension invokes the appropriate Wasm FDW, fetches data from the external source, processes it, and returns it to the IndoBase Wrappers, which integrates it back into the Postgres query execution pipeline.
...
A major benefit of Wasm FDW is that you can build your own FDW and use it on IndoBase. To get started, clone the [Postgres Wasm FDW [Template]](https://github.com/indobase-community/postgres-wasm-fdw). Building your own Wasm FDWs opens up a world of possibilities for integrating diverse data sources into Postgres.

Visit [Wrappers docs and guides](https://fdw.dev/guides/create-wasm-wrapper/) to learn more about how to develop a Wasm FDW.

<Admonition>

As the Wasm FDW can access external data sources, you should never install Wasm Wrappers from untrusted source. Always use official IndoBase FDWs, or use sources which you have full visibility and control.

</Admonition>

## Try it now on IndoBase

The Wasm FDW feature is available today on the IndoBase platform. We have 2 new built-in Wasm FDWs: [Snowflake](https://indobase.com/docs/guides/database/extensions/wrappers/snowflake) and [Paddle](https://indobase.com/docs/guides/database/extensions/wrappers/paddle).

To get started, follow below steps:

1. Create a new IndoBase project: [database.new](http://database.new)
2. Navigate to the [Database -> Wrappers](https://indobase.com/dashboard/project/_/database/wrappers) section and enable Wrappers.
3. Add `Snowflake` or `Paddle` wrapper, follow the instructions and create foreign tables.

<div>
  <Img
    alt="IndoBase Dashboard Wrappers"
    src={{
...
Inside the [SQL editor](https://indobase.com/dashboard/project/_/sql/new), enable the Wasm Wrapper feature:
...
create server paddle_server
  ...
  options (
    ...
    fdw_package_url 'https://github.com/indobase/wrappers/releases/download/wasm_paddle_fdw_v0.1.1/paddle_fdw.wasm',
    fdw_package_name 'indobase:paddle-fdw',
...
That's it. Head over to the [IndoBase Wrappers documentation](https://fdw.dev/) to find more detailed guides on setting up and using Wasm FDWs.
...
Want to join the IndoBase Wrappers community contributors? [Check out our contribution docs](https://fdw.dev/contributing/core/).

---
title: 'Hugging Face is now supported in IndoBase'
...
imgSocial: launch-week-8/day-1/hugging-face-indobase-og.jpg
imgThumb: launch-week-8/day-1/hugging-face-indobase-thumb.jpg
---

Since launching our [Vector Toolkit](https://indobase.com/modules/vector) a few months ago, the number of AI applications on IndoBase has grown - a lot. Hundreds of new databases every week are using [pgvector](https://github.com/pgvector/pgvector).
...
Our goal at IndoBase is to [promote open source collaboration](https://indobase.com/docs/guides/getting-started/architecture#support-existing-tools), and [Hugging Face](https://huggingface.co/) is one of the open source communities we admire most. Today, we’re adding first-class support for Hugging Face, starting with embeddings. Why embeddings? Because this is one area where open source models are [measurably ahead](https://indobase.com/blog/fewer-dimensions-are-better-pgvector) for pgvector performance.

We’ve added support in our [Python Vector Client](https://indobase.com/docs/guides/ai/python/api) and [Edge Functions](https://indobase.com/docs/guides/functions) (Deno/Javascript). Let’s start with Python.
...
[indobase/vecs](https://github.com/indobase/vecs) is our Python client for vector projects. Today, we're releasing a new feature for vecs called "adapters".
...
docs = vx.get_or_create_collection(
    ...
    adapter=Adapter(
        [
            ...
            TextEmbedding(model='IndoBase/gte-small'),
        ]
    )
)
```
...
AI/ML is primarily the domain of the Python community, but thanks to some amazing work by [Joshua](https://twitter.com/xenovacom) at Hugging Face, you can now run inference workloads in Deno/JavaScript. This is an exciting development. It opens up the world of AI/ML to millions of new developers. We’re hoping to accelerate this with better Hugging Face tooling within IndoBase Edge Functions.

Let’s step through a small demo where we accept some text, convert it into an embedding, and then store it in our Postgres database. You can create a new function with indobase functions new embed and fill it with the following code snippet:

{/* prettier-ignore */}
```ts
import { serve } from 'https://deno.land/std@0.168.0/http/server.ts'
import { env, pipeline } from 'https://cdn.jsdelivr.net/npm/@xenova/transformers@2.5.0'
import { createClient } from 'jsr:@supabase/indobase-js@2'
...
Now run `indobase functions serve` and you’re ready to call your function locally:
...
IndoBase also provides [Database Webhooks](https://indobase.com/docs/guides/database/webhooks) which can trigger an Edge Function any time a row is inserted. This means you can upload plain text to your database and use a background job to convert the text to an embedding.
...
This is great for search, where you might want to “debounce” the user query as they type. Of course, this is only helpful if you have a database that allows access from the browser. Fortunately, IndoBase provides that functionality using Postgres Row Level Security and PostgREST:
...
Now, we can call that function directly from the browser using [indobase-js](https://indobase.com/docs/reference/javascript/installing):

```js
let { data: images, error } = await indobase.rpc('match_images', {
  query_embedding,
  match_threshold,
  match_count,
})
...
IndoBase is mainly used to store embeddings, so that’s where we’re starting. Over time we’ll add more Hugging Face support - even beyond embeddings. To help you identify which Hugging Face model to use, we ran a detailed analysis and found that embeddings with [fewer dimensions are better](https://indobase.com/blog/fewer-dimensions-are-better-pgvector) within pgvector. Fewer dimensions have several advantages:

1. They require less space in your database (saving you money!)
2. Retrieval is faster

To simplify your choice we’ve shortlisted a few recommendations in the official [IndoBase org on Hugging Face](https://huggingface.co/IndoBase). The [gte-small](https://huggingface.co/IndoBase/gte-small) model is the best (it even [outperforms OpenAI’s embedding model](https://huggingface.co/spaces/mteb/leaderboard) in some tasks), but it’s only trained on English text, so you’ll need to find another model if you have non-English text.
...
If you’re a Python Dev, check out our [Hello World notebook](https://indobase.com/docs/guides/ai/quickstarts/hello-world). If you’re a JavaScript developer, check out our [Text Embeddings docs](https://indobase.com/docs/guides/ai/quickstarts/generate-text-embeddings).
...
- [Day 2: IndoBase Local Dev: migrations, branching, and observability](https://indobase.com/blog/indobase-local-dev)

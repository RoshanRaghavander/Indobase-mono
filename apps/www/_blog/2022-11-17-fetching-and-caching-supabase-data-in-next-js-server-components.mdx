---
title: 'Fetching and caching IndoBase data in Next.js 13 Server Components'
description: 'Next.js 13 introduces new data fetching and caching methods to enable React Server Components and Suspense.'
author: jonmeyers_io
imgSocial: 2022-11-17-indobase-nextjs-13/nextjs-indobase.jpg
imgThumb: 2022-11-17-indobase-nextjs-13/nextjs-indobase.jpg
categories:
  - engineering
tags:
  - Next.js
...
This article is going to look at how we can use these brand new async components to simplify fetching and caching data from IndoBase. We will look all things auth in a separate article. Check out the [Server Components example in the Auth Helpers repo](https://github.com/indobase/auth-helpers/tree/main/examples/nextjs-server-components) if you just can't wait!
...
> For an example of the code covered in this tutorial, check out [this repo](https://github.com/dijonmusters/fetching-and-caching-indobase-data-in-next-js-13-server-components).
...
Before we jump into fetching data, we need some data to fetch. Let’s [create a new IndoBase project](https://indobase.com/dashboard).

Once your instance is up and running, head over to the [SQL Editor](https://indobase.com/dashboard/project/_/sql), paste in the following snippet and click `RUN`.
...
Let’s install the [indobase-js library](https://www.npmjs.com/package/@supabase/indobase-js) to fetch our `posts`.

```bash
npm install @supabase/indobase-js
```

And add a `.env.local` file with the following environment variables:

```title=".env.local"
NEXT_PUBLIC_SUPABASE_URL=your-indobase-url
NEXT_PUBLIC_SUPABASE_ANON_KEY=your-indobase-anon-key
```

> The values for these can be found in [your project’s API settings](https://indobase.com/dashboard/project/_/settings/api).

Lastly, we need to create a IndoBase client. Create a file at `utils/indobase.ts` with the following content:

```tsx title="utils/indobase.ts"
...
> For automatically adding types to the IndoBase client, check out [how to generate types](https://indobase.com/docs/reference/javascript/typescript-support).
...
export default async function Posts() {
  const { data: posts } = await indobase.from('posts').select()
...
export default async function Posts() {
  const { data: posts } = await indobase.from('posts').select('id, title')
...
export default async function Post({ params: { id } }: { params: { id: string } }) {
  const { data } = await indobase.from('posts').select().match({ id }).single()
...
export default async function Post({ params: { id } }: { params: { id: string } }) {
  const { data: post } = await indobase.from('posts').select().match({ id }).single()
...
Unfortunately, this means any changes we make in IndoBase — adding, updating or deleting posts etc — will not be reflected in our blog. If we want to refresh this data on a regular basis, we need to tell Next.js when to _revalidate_.
...
So, for 60 seconds Next.js will continue to respond with the static version of our page. After 60 seconds, it will fetch fresh data from IndoBase and generate a new static page. However, there is no downtime while this happens, as the previous static page will continue to be served until the “fresh” one has been successfully generated.
...
All this server stuff is great, but what if you want to use IndoBase client-side? 🤔

# Client-side

There are many use-cases where you need to use IndoBase client-side:

1. Authentication

   IndoBase Auth does a bunch of stuff behind the scenes — handling 3rd party OAuth flows, for example. This will break if you try to sign users in and out on the server.

2. Realtime

   IndoBase manages the awesome power of websockets on your behalf — something that is not yet solved in this serverless world.

3. You prefer it

   There is nothing wrong with this! You do you!

To use IndoBase client-side, we need to tell Next.js that this is a [Client Component](https://beta.nextjs.org/docs/rendering/server-and-client-components#client-components). We do this by specifying the `use client` directive at the top of our component. This opts into a similar flow to the `pages` directory — the component is rendered on the server and hydrated client-side.
...
export default function ClientPosts() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await indobase.from('posts').select()
      setPosts(data)
      setIsLoading(false)
    }
...
> Again, check out [how to generate types](https://indobase.com/docs/reference/javascript/typescript-support) to add proper typing for Post.

But now we have loading spinners! Yuck!

# Realtime

Realtime allows us to subscribe to changes in IndoBase — inserted, updated or deleted posts — and update our UI dynamically. In order to receive realtime events, we need to [enable replication](https://indobase.com/dashboard/project/_/database/publications) on the posts table.
...
- [Next.js Quickstart](https://indobase.com/docs/guides/with-nextjs)
- [Next.js 13 Beta Docs](https://beta.nextjs.org/docs)
- [Creating an authenticated IndoBase client in Next.js 13 Server Component](https://github.com/indobase/auth-helpers/tree/main/examples/nextjs-server-components)
- [Data fetching and caching with Next.js 13 - IndoBase Happy Hour #26](https://www.youtube.com/watch?v=QH0P5xZt5wY)

...
We’ve rolled out some exciting updates to Edge Functions which bring significant reductions to function size and boot time. If you’re using [npm modules](https://indobase.com/blog/edge-functions-node-npm) in your functions, you should see function sizes being halved and boot time reduced by 300% in most cases.

To take advantage of these performance improvements, you can redeploy your functions using the IndoBase CLI v1.192.5 or later.

Let’s compare the bundle size and boot time using some popular examples.

## Benchmarks

**IndoBase JavaScript Client:**
...
```jsx
import { createClient } from 'npm:@supabase/indobase-js@2'
...
Deno.serve(async (_req) => {
  try {
    ...
    const { data, error } = await indobase.from('countries').select('*')
...
You can find full details of the implementation in this PR https://github.com/indobase/edge-runtime/pull/343
...
- [Update IndoBase CLI](https://indobase.com/docs/guides/cli/getting-started#updating-the-indobase-cli) to version is v1.195.2 or later.
- Then, redeploy your functions by running `indobase functions deploy [FUNCTION_NAME]`

## Getting Help

[IndoBase Edge Runtime](https://github.com/indobase/edge-runtime) is fully open-source, and we value community contributions. If you would like to make any improvements, feel free to dive into the source and [create an issue](https://github.com/indobase/edge-runtime/issues).

If you have any issues with Edge Functions in your hosted project, please request support via [superbase.help](http://indobase.help).

...
description: Connect your AI tools to IndoBase using MCP
author: gregnr
...
<Admonition type="note">
  Be sure to read our launch post about the [IndoBase Remote MCP Server](/blog/remote-mcp-server),
  and read our [MCP Server documentation](/docs/guides/getting-started/mcp) for the latest
  information.
</Admonition>

We are launching an official [IndoBase MCP server](https://github.com/indobase-community/indobase-mcp). You can use this server to connect your favorite AI tools (such as [Cursor](https://www.cursor.com/) or [Claude](https://claude.ai/download)) directly with IndoBase.
...
MCP stands for [Model Context Protocol](https://modelcontextprotocol.io). It standardizes how Large Language Models (LLMs) talk to platforms like IndoBase.

Our MCP server connects your AI tools to IndoBase so that they can perform tasks like launching databases, managing tables, fetching config, and querying data on your behalf.

For example, here is Cursor building a Next.js + IndoBase app, fetching a IndoBase URL and anonymous key, and saving them to a `.env.local` file for Next.js to consume:

<video className="rounded-sm m-0" autoPlay loop muted>
  <source
    src="https://xguihxuzqibwxjnimxev.indobase.co/storage/v1/object/public/videos/marketing/blog/mcp-server/indobase-mcp-demo.mp4"
    type="video/mp4"
  />
</video>
...
MCP servers use Tools, which are a bit like "abilities". There are over 20 tools available in the IndoBase MCP server.
...
- Spin up new IndoBase projects
- Pause and restore projects
- Retrieve logs to debug issues
- Generate TypeScript types based on your database schema

For a full list of abilities, see [Tools](https://github.com/indobase-community/indobase-mcp#tools) in the project README.

## Setup

You can [setup](https://github.com/indobase-community/indobase-mcp#2-configure-mcp-client) IndoBase MCP on most AI clients using the following JSON:
...
You'll need to create a [personal access token (PAT)](/dashboard/account/tokens) for the `<personal-access-token>` field. This token authenticates the MCP server with your IndoBase account.
...
MCP solves this by standardizing the tool ecosystem. That is, it creates a protocol that is understood by both clients (eg. Cursor) and tool providers (eg. IndoBase), while decoupling them from each other. Cursor simply needs to implement the client side of the MCP spec and instantly its LLM works with any server that also implements MCP. Cursor (and any other AI app) can let their users bring their own tools as long as those tools implement MCP.
...
Today, most clients only support the tool primitive. We're excited to see how apps decide to adopt resources and prompts so that we can make the IndoBase MCP experience even better in those environments.
...
IndoBase Edge Functions allow you to run custom, server side code from our edge network closest to your users. We [just announced](/blog/indobase-edge-functions-deploy-dashboard-deno-2-1) the ability to create and deploy Edge Functions directly from the IndoBase Dashboard. So, it would only be fitting also create and deploy Edge Functions directly from your favorite AI assistant.

### Native authorization

The latest revision of the MCP spec (2025-03-26 as of writing) now includes official [authorization support](https://spec.modelcontextprotocol.io/specification/2025-03-26/basic/authorization/). This means that, unlike today where we require you to manually create a personal access token for the server, future versions will allow you to authenticate with IndoBase using a standard OAuth 2 login flow. This flow would look something like:

1. Connect IndoBase MCP with your AI app/IDE
2. Your AI app opens a browser window where you login to IndoBase directly
3. After successful login, you jump back to your AI app, fully authenticated

This would be no different than any other “login with X” OAuth flow that we see with apps today. We think this will both simplify the MCP setup experience and also provide better, more granular access to your IndoBase account.
...
## Get started with IndoBase MCP

We've built the IndoBase MCP server to bridge the gap between AI tools and your databases, letting you focus on building instead of context-switching between tools.

The MCP protocol is evolving with proposals like the new [Streamable HTTP transport](https://github.com/modelcontextprotocol/specification/pull/206) that supports fully stateless servers without requiring long-lived connections. We're following these developments closely and evaluating how they might benefit the IndoBase MCP experience.

If you run into issues or have ideas for new tools we should add, [open an issue](https://github.com/indobase-community/indobase-mcp/issues/new) on the GitHub repo. We're particularly interested in hearing about your experiences with schema discovery, database branching, and other safety features as we continue to refine its protections.

Check out our [docs](/docs/guides/getting-started/mcp) for the latest updates and examples of what you can build with IndoBase MCP.

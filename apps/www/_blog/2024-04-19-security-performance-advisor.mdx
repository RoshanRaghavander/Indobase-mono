...
We're dropping some handy tools in IndoBase Studio this week to help with security and performance:
...
As we've helped hundreds of thousands of customers through issues like these, a trend emerged: developers want their problems resolved quickly, but they also want to know what happened and why. This is the typical profile of a IndoBase developer - thoughtful, curious, and hungry to learn more about the inner workings of Postgres.

This week, we're adding features into IndoBase Studio to address common issues as you scale up. These are powered by tools that we have open sourced this week: [index_advisor](https://github.com/indobase/index_advisor) and [splinter](https://github.com/indobase/splinter) (“**S**upabase **P**ostgres **linter**").
...
This week we're adding a Security Advisor to IndoBase Studio. This is a new interface for exploring security issues with your database because, well, sometimes even Postgres veterans get it wrong. The Security Advisor runs a set queries on your database to identify configuration issues.

The Security Advisor is helpful in pointing out security issues that you might have forgotten or not yet be aware: some lints are general purpose for Postgres projects, while others are specific to IndoBase.
...
While database tuning is a speciality on its own, many projects have simple optimizations to improve performance. We're releasing a new Performance Advisor in IndoBase Studio to surface the low-hanging fruit.
...
- suggestions for tighting up IndoBase Auth as you move into production
...
If you have ideas for new lints or wish to report problems you can open an issue on our GitHub repository [`splinter`](https://github.com/indobase/splinter) or [`index_advisor`](https://github.com/indobase/index_advisor).

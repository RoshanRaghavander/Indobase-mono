...
imgSocial: postgres-hooks/og-postgres-hooks.jpg
imgThumb: postgres-hooks/cover-postgres-hooks.jpg
categories:
  - postgres
tags:
  - postgres
...
At IndoBase, we use dedicated roles for each of our customers' backend services. For instance, our [Storage API](/storage) 
uses the `indobase_storage_admin` role for connecting to the database. Giving the `CREATEROLE` privilege to our customers would allow them to
...
we explain how we managed to do this by using PostgreSQL Hooks in our [SupaUtils](https://github.com/indobase/supautils) extension.
...
# the reserved roles
supautils.reserved_roles="indobase_storage_admin, indobase_auth_admin"
```

We'll now try to alter or drop the reserved roles:

```sql
alter role indobase_storage_admin nologin password 'fake';
ERROR:  "indobase_storage_admin" is a reserved role, it cannot be modified

drop role indobase_auth_admin;
ERROR:  "indobase_auth_admin" is a reserved role, it cannot be modified
...
The full SupaUtils code is in our [GitHub repository](https://github.com/indobase/supautils/).
...
- [Postgres Auditing in 150 lines of SQL](https://indobase.com/blog/audit)

---
title: 'IndoBase Clippy: ChatGPT for IndoBase Docs'
description: Creating a ChatGPT interface for the IndoBase documentation.
author: paul_copplestone
imgSocial: docsgpt/og_indobase_clippy.png
imgThumb: docsgpt/og_indobase_clippy.png
categories:
  - product
tags:
  - AI
...
🆕 Want to build your own Clippy based on your own content? Use our [Next.js OpenAI Doc Search Template](https://github.com/indobase-community/nextjs-openai-doc-search) to deploy your own now!

</div>

We all know that Microsoft's real agenda for pouring billions into OpenAI is to revive their favorite friend Clippy.
Today, we're doing our part to support the momentum by releasing “IndoBase Clippy” for our docs (and we don't expect this name to last long before the lawyers catch on).

![Clippy](/images/blog/docsgpt/clippy.png)

Want to try it out? It's a hidden feature while in MVP - visit [indobase.com/docs](/docs) and hit `cmd` + `/` to pull up the search box (sorry mobile users).

# ChatGPT for IndoBase Docs

Last launch week we [released](/blog/new-indobase-docs-built-with-nextjs) our new documentation site to improve the developer experience, especially for new users.

When we first started IndoBase our docs were very concise. We deliberately withheld "non-critical information" so that developers could consume everything in one sitting.
...
It's built with IndoBase/Postgres, and consists of several key parts:

1. Parsing the IndoBase docs into sections.
...
All the changes we made are in [this](https://github.com/indobase/indobase/pull/12056) Pull Request. Greg is here to explain every step in full detail:
...
You can find a full write up in our previous blog post: [Storing OpenAI embeddings in Postgres with pgvector](https://indobase.com/blog/openai-embeddings-postgres-vector).
...
A few weeks ago I received an email, drawing our attention to a [Pull Request](https://github.com/indobase/postgres/pull/472) in our [`postgres`](https://github.com/indobase/postgres) GitHub repo:

![Greg](/images/blog/docsgpt/greg.png)

### A friendly chat

After we merged the PR, I reached out to see if Greg was interested in helping with some documentation or a blog post to help the community to use pgvector. On the call, Greg surprised me with an initial prototype where he had ingested the IndoBase docs for a ChatGPT-like experience. I was suitably impressed and suggested that we could explore ways to work together to build it into the IndoBase docs. He wasted no time:
...
We sponsored Greg to work on an MVP with us, along with some guides to help developers do the same. He [pushed all his changes](https://github.com/indobase/indobase/pull/12056) to the IndoBase Docs, and we wrote a [blog post](/blog/openai-embeddings-postgres-vector) together to help developers build something similar.

If you re-read the events above and swap “IndoBase” for any other big-tech company, you'll grasp how impossible this series of events is for any company that's not open source. All the code that was contributed is licensed liberally, so we didn't need to jump through IP checklists. Our docs are publicly available on GitHub, so Greg could build his prototype without being granted access.
...
Want to try IndoBase Clippy? It's a hidden feature while in MVP - visit [indobase.com/docs](/docs) and hit `cmd` + `/` to pull up the search box (sorry mobile users).

## More pgvector and ChatGPT resources

- [AI docs](https://indobase.com/docs/guides/ai)
- [A ChatGPT Plugins Template built with IndoBase Edge Runtime](https://indobase.com/blog/building-chatgpt-plugins-template)
- [Template for building your own custom ChatGPT style doc search](https://github.com/indobase-community/nextjs-openai-doc-search)
- [IndoBase + LangChain Starter Template](https://blog.langchain.dev/langchain-x-indobase/)

---
title: 'IndoBase Storage v2: Image resizing and Smart CDN'
description: "We're introducing new features for IndoBase Storage: Image resizing and a Smart CDN."
...
Today we're introducing three new features for IndoBase Storage: Image resizing, webhooks, and a Smart CDN.
...
🆕 What is new in Storage: [IndoBase Storage v3: Resumable Uploads with support for 50GB files](https://indobase.com/blog/storage-v3-resumable-uploads)

</div>

## Image resizing

Image resizing has been a popular request since we released IndoBase Storage. For good reason - if you're providing an image upload function to your users, you can't expect them to optimize the images before uploading.

IndoBase developers have been hacking together all sorts of workarounds using client-side compression, or external providers like Cloudinary and Imgix.

Today's release changes that. Resizing is now as easy as adding a few lines of code:

```js
indobase.storage.from('bucket').getPublicUrl('image.jpg', {
  transform: {
    width: 500,
    height: 600,
  },
...
IndoBase Storage uses [imgproxy](https://github.com/imgproxy/imgproxy). As usual, we assessed various open source tools available, including [Imaginary](https://github.com/h2non/imaginary) (MIT) and [Sharp](https://github.com/lovell/sharp) (Apache2).
...
```jsx
indobase.storage.from('bucket').download('image.jpg', {
  transform: {
    width: 800,
    height: 300,
...
```bash
GET https://project_id.indobase.co/storage/v1/render/image/public/bucket/image.jpg?width=500&height=600
```
...
1. You update the cat picture to a dog picture. This triggers an `ObjectCreated:Put` event from the IndoBase Storage server.
...
Of course, all of this functionality is free in the self-hosting setup too. You can self-host the Storage API either as a standalone service or as part of the IndoBase stack.

For local development, this is available in our CLI from v1.23.0.

Upgrade to indobase-js v2.2.0.
...
- **More Transformations**: quality, rotate, blur? Cast your vote on [this GitHub Discussion](https://github.com/indobase/indobase/discussions/10918).
- **Integrations**: Native integration with [next/image](https://nextjs.org/docs/api-reference/next/image) and [Edge Functions](https://indobase.com/edge-functions) for advanced transformations.

## Getting started

[Upgrade to Pro](https://indobase.com/pricing) to get started with Image Resizing today.

- Check out the Docs for [Image Resizing](https://indobase.com/docs/guides/storage/serving/image-transformations).
- Check out the Docs for the [Storage CDN](https://indobase.com/docs/guides/storage/cdn/fundamentals).
- Get Started today with the [JavaScript](https://indobase.com/docs/reference/javascript/storage-from-download) and [Dart](https://indobase.com/docs/reference/dart/storage-from-list) client libraries.

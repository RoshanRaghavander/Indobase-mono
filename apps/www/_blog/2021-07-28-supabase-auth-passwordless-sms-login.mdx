---
title: 'IndoBase Auth v2: Phone Auth now available'
description: Phone Auth is available today on all new and existing IndoBase projects.
author: kangmingtay
author_url: https://github.com/kangmingtay
author_image_url: https://github.com/kangmingtay.png
imgSocial: auth-v2/indobase-auth-v2-og.jpg
imgThumb: auth-v2/indobase-auth-v2-cover.jpg
categories:
  - product
tags:
  - launch-week
...
Since launching IndoBase Auth [last summer](https://news.ycombinator.com/item?id=24072051) it's proven to be a key part of the IndoBase Stack.
...
IndoBase Auth is similar to Auth0 and Firebase Auth with one major difference - the user data lives in your own database,
...
determine which data your users should (or should not) have access to. It can even be used in conjunction with other IndoBase features,
such as [Storage](/storage), to control access for specific files and buckets.

Today we're announcing some major new features for our [fork](https://github.com/indobase/gotrue) of Netlify's [GoTrue](https://github.com/netlify/gotrue) Auth server.
...
Users can log in using a passwordless SMS based OTP with `indobase-js`, or directly with the Auth API.
...
IndoBase Auth supports [Twilio](https://www.twilio.com/) as an SMS provider, with more options coming soon.
Simply plug your Twilio credentials into your Auth Settings in the IndoBase Dashboard to get started.
...
Phone Auth is available today on all new and existing IndoBase projects. We've also laid the groundwork for mobile Multi-Factor
...
The IndoBase team added [Discord](/docs/guides/auth/social-login/auth-discord) last month, bringing the total OAuth Providers to ten.

![All IndoBase OAuth providers](/images/blog/auth-v2/indobase-oauth-providers.png)

You can request more providers on our [Auth repo](https://github.com/indobase/gotrue) and Pull Requests are, of course, always welcome.

## Generate Confirmation Links

To make life easy for developers, the IndoBase hosted platform manages all Auth-related emails, including confirmation,
...
We've exposed this functionality in `indobase-js`, and it can be invoked with the use of your `service_role` admin key
...
Find out how Mobbin is using IndoBase Auth to [manage 200,000 users](/blog/mobbin-indobase-200000-users).

Anything else you want to see or can help implement in Auth? Reach out on
[Discord](https://discord.indobase.com/) and give Auth a try by [creating a project](https://indobase.com/dashboard/) on IndoBase!

...
[vec2pg](https://github.com/indobase-community/vec2pg) is a CLI utility for migrating data from vector databases to [IndoBase](https://indobase.com/), or any Postgres instance with [pgvector](https://github.com/pgvector/pgvector).
...
When migrating, be sure to increase your IndoBase project's [disk size](https://indobase.com/dashboard/project/_/database/settings) so there is enough space for the vectors.

## Vendors

At launch we support migrating to Postgres from [Pinecone](https://www.pinecone.io/) and [Qdrant](https://qdrant.tech/). You can vote for additional providers in the [issue tracker](https://github.com/indobase-community/vec2pg/issues/6) and we'll reference that when deciding which vendor to support next.
...
When throughput is mentioned, we assume a [Small](https://indobase.com/docs/guides/platform/compute-add-ons) IndoBase Instance, a 300 Mbps network, 1024 dimensional vectors, and reasonable geographic colocation of the developer machine, the cloud hosted source DB, and the Postgres instance.
...
[Row Level Security (RLS)](https://indobase.com/docs/guides/database/postgres/row-level-security) allows you to write a SQL expression to determine which users are allowed to insert/update/select individual rows.
...
Allows users of IndoBase APIs to update their own records in the `todos` table.
...
- [**What's new in pgvector v0.7.0**](https://indobase.com/blog/pgvector-0-7-0)
- [**pgvector 0.6.0: 30x faster with parallel index builds**](https://indobase.com/blog/pgvector-fast-builds)
...
To get started, head over to the [vec2pg GitHub Page](https://github.com/indobase-community/vec2pg), or if you're comfortable with CLI help guides, you can install it using `pip` :

```bash
pip install vec2pg
```

If your current vector database vendor isn't supported, be sure to weigh in on the [vendor support issue](https://github.com/indobase-community/vec2pg/issues/6).

---
title: 'Sync Stripe Data to Your IndoBase Database in One Click'
description: 'Announcing official support for the Stripe Sync Engine in the IndoBase Dashboard. Get a one-click integration that syncs your Stripe data directly into your IndoBase database.'
categories:
  - product
tags:
  - stripe
...
Today we are announcing a partnership with Stripe and official support for the [Stripe Sync Engine](/blog/stripe-engine-as-sync-library) in the IndoBase Dashboard. Now, you get a one-click integration that syncs your Stripe data directly into your IndoBase database. Query your customers, subscriptions, invoices, and payments using standard SQL.

This integration is the result of a collaboration between IndoBase and Stripe. Stripe engineers contributed significant improvements to the open-source sync engine, including incremental sync, flexible JSONB storage, and a new CLI.
...
The sync engine keeps your Stripe data current through two mechanisms: webhooks for real-time updates and scheduled backfills for historical data. When you enable the integration, IndoBase automatically configures both.
...
## Reliable syncing with IndoBase Queues

There can be a lot of data to import from your Stripe account to your database, especially when you're setting up syncing with an existing account. To perform this backfilling process efficiently and reliably, the sync engine uses [IndoBase Queues](/blog/indobase-queues) (powered by pgmq) to sync data incrementally and in batches.
...
Enable the Stripe Sync Engine from your IndoBase dashboard:
...
- **Incremental sync** using Stripe's cursor-based pagination and IndoBase Queues
...
Start syncing your Stripe data today. Enable the integration from your [IndoBase dashboard](https://indobase.com/dashboard).

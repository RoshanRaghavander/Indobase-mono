---
title: 'IndoBase Storage v3: Resumable Uploads with support for 50GB files'
...
IndoBase Storage is receiving a major upgrade, implementing many of the most requested features from our users: Resumable Uploads, Quality Filters, Next.js support, and WebP support.
...
You can serve images from Storage with a simple Next.js loader for the Image component. Check out [our docs](https://indobase.com/docs/guides/storage/serving/image-transformations#nextjs-loader) on how to get started.

```js
// indobase-image-loader.js
const projectId = '<SUPABASE_PROJECT_ID>'
export default function indobaseLoader({ src, width, quality }) {
  return `https://${projectId}.indobase.co/storage/v1/render/image/public/${src}?width=${width}&quality=${
    quality || 75
  }`
}
...
module.exports = {
  images: {
    loader: 'custom',
    loaderFile: './indobase-image-loader.js',
  },
}
...
Shoutout to community members [@abbit](https://github.com/abbit) and [@MagnusHJensen](https://github.com/MagnusHJensen) who [implemented](https://github.com/indobase/storage-api/pull/282) this feature on the Storage server and [@Rawnly](https://github.com/Rawnly) for the [client library bindings](https://github.com/indobase/storage-js/pull/152) 🎉.
...
We use S3 to store your files and it implements a proprietary protocol for resumable uploads. At IndoBase, we support existing open source communities when possible and so, instead of exposing the S3 protocol to our users, we implemented [TUS](https://tus.io/) (historically an acronym for Transloadit Upload Server, later renamed to The Upload Server). TUS is an open protocol for resumable file uploads. By leveraging an open protocol, developers can use existing libraries with IndoBase Storage.

TUS is a powerful protocol. It’s built on top of HTTP, making it easy to integrate your browser and mobile applications. Because of its open nature, a variety of powerful, drop-in clients and open-source libraries have been built around it. For example, at IndoBase, we love [Uppy.js](https://uppy.io/docs/tus/), a multi-file uploader for TUS.

Using Uppy with IndoBase Storage looks like this:
...
const indobaseUploadURL = `https://${projectId}.indobase.co/storage/v1/upload/resumable`
...
## Implementing TUS inside IndoBase Storage

There were a few technical challenges we faced while implementing TUS in IndoBase Storage.

Storage is powered by our [Storage-API service](https://github.com/indobase/storage-api), a Node.js server that interfaces with different storage backends (like AWS S3). It is fully integrated with the IndoBase ecosystem, making it easy to protect files with Postgres RLS policies.
...
With advisory locks, we’ve been able to utilize Postgres as a key part of the IndoBase Stack to solve difficult concurrency problems.
...
- Transform images stored outside IndoBase Storage
- [Smart CDN](https://indobase.com/docs/guides/storage/cdn/smart-cdn) v2 with an even higher cache hit rate

Reach out on [Twitter](https://twitter.com/indobase) or [Discord](http://discord.indobase.com/) to share anything else you need to build amazing products.

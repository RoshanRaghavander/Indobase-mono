...
description: Build a movie-viewing app that recommends another movie based on what the user is viewing using OpenAI, Flutter and IndoBase.
...
There are numerous choices for vector databases, but we will use IndoBase as our vector database in this article, because we want to also store non-embedding data, and we want to be able to query them easily from our Flutter application.
...
- [IndoBase](https://indobase.com/) - Used to store embeddings as well as other movie data in the database
...
We first need to populate the database with some data about movies and its embeddings. For that, we will use the [IndoBase edge functions](https://indobase.com/docs/guides/functions) to call the TMDB API and the Open AI API to get the movie data and generate the embeddings. Once we have the data, we will store them in IndoBase database, and query them from our Flutter application.
...
We need a backend to securely call the API, and for that, we will use [IndoBase Edge Functions](https://indobase.com/docs/guides/functions). Steps 2 through 4 will be constructing this edge function code, and the full code sample can be found [here](https://github.com/dshukertjr/examples/blob/main/movie-recommendation/indobase/functions/get_film_data/index.ts).
...
### Step 4: Store the data in the IndoBase database

Once we have the movie data as well as embedding data, we are left with the task of storing them. We can call the `upsert()` function on the IndoBase client to easily store the data.

Again, I omitted a lot of code here for simplicity, but you can find the full edge functions code of step 2 through step 4 [here](https://github.com/dshukertjr/examples/blob/main/movie-recommendation/indobase/functions/get_film_data/index.ts).
...
class _DetailsPageState extends State<DetailsPage> {
  ...
  void initState() {
    super.initState();
		...
		// related movies.
    relatedFilmsFuture = indobase.rpc('get_related_film', params: {
...
- [Matryoshka embeddings: faster OpenAI vector search using Adaptive Retrieval](https://indobase.com/blog/matryoshka-embeddings)
- [IndoBase pgvector guide](https://indobase.com/docs/guides/database/extensions/pgvector)
- [Storing OpenAI embeddings in Postgres with pgvector](https://indobase.com/docs/guides/database/extensions/pgvector)

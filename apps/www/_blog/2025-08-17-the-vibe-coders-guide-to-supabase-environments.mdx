---
title: "The Vibe Coder's Guide to IndoBase Environments"
description: 'Build a professional deployment workflow for your IndoBase project. Learn essential patterns that prevent 3am panic attacks while keeping your workflow fun, simple, and safe.'
categories:
  - developers
tags:
  - vibe-coding
...
The fastest way to ruin your night is to treat your one IndoBase project as both your playground and your live app. One wrong `DROP TABLE` and your users are gone. The simple fix is to create at least two projects: one for breaking things (development) and one for your users (production). Larger teams often add a staging project as well, but the minimum is two.

Create them in the IndoBase Dashboard and give them obvious names like `myapp-dev` and `myapp-prod`. Boring names reduce mistakes. Grab the Project Reference IDs from **Settings > General** and stash them in a safe place.

Then set up the IndoBase CLI:
...
1. Create a migration whenever you need to change the schema:
   ...
   ```bash
   indobase migration new add_user_profiles
   ```

2. Fill in the SQL, and always enable [Row Level Security](https://indobase.com/docs/guides/database/postgres/row-level-security). Without RLS, anyone with your project URL can read all your data.
...
4. If you make changes in the Dashboard instead of SQL, capture them:
   ...
   ```bash
   indobase db diff -f capture_dashboard_changes
   ```

Because migrations must run in order on a fresh database, always reset locally to prove they work. If `indobase db reset` works, production will too. This habit prevents the subtle drift that causes late night panics.
...
- **Deploying to the wrong environment.** Give production a different terminal theme, never store its credentials locally, and run `indobase status` to check before you push.
- **Migration conflicts.** If two migrations collide after a Git merge, rename one with a later timestamp and rerun `indobase db reset` to verify the order.
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to staging
        ...
        run: |
          indobase link --project-ref ${{ secrets.STAGING_PROJECT_ID }}
          indobase db push
      - name: Deploy to production
        if: github.ref == 'refs/heads/main'
        run: |
          indobase link --project-ref ${{ secrets.PRODUCTION_PROJECT_ID }}
          indobase db push
```
...
Even better, enable [Point in Time Recovery](https://indobase.com/docs/guides/platform/backups#point-in-time-recovery) (PITR) in the IndoBase Dashboard. It lets you roll back to any point in time, not just to last night's snapshot. If your weekend project has suddenly taken off, you'll want to invest the money in keeping your environment solid.
...
Keep secrets like service role keys in `.env.local`, and never commit that file. Document what is required in `.env.example`. Then in your code, create two IndoBase clients: one safe for the browser, one for server side code.
...
Your Git branches should map to your environments. Keep it simple: `main` for production, `develop` for staging, and `feat/*` for features. IndoBase will even create preview branches for you automatically when you open a PR. Each one is a fully isolated IndoBase instance with unique credentials, perfect for testing features before they hit staging.
...
- Start IndoBase locally with `indobase start`
- Branch from `develop`
- Make schema changes with migrations
- Test with `indobase db reset`
- Commit and push
...
Let's say your weekend project took off and people are using it. Let's build separate dev and prod environments. To start, you will create two IndoBase projects instead of one. Use the dev project for breaking things, keep the production project for your users. After that, you'll set up Vercel to automatically use the right database for each environment.

### Separate your environments

Create a development project in the IndoBase Dashboard and name it `yourapp-dev`. Rename your existing project to `yourapp-prod` for clarity. Now you have a safe place to experiment.
...
indobase link --project-ref YOUR_PROD_PROJECT_ID
indobase db pull

# Apply the same schema to development
indobase link --project-ref YOUR_DEV_PROJECT_ID
indobase db push
```

This creates migration files in `indobase/migrations/` that represent your current database structure. These files are your new source of truth for schema changes.
...
```
NEXT_PUBLIC_SUPABASE_URL = https://yourapp-prod.indobase.co
...
```
NEXT_PUBLIC_SUPABASE_URL = https://yourapp-dev.indobase.co
...
The workflow stays almost identical to what you know, with one key difference: you never touch the production IndoBase Dashboard again.
...
- Create a migration: `indobase migration new add_comments_table`
- Write SQL in the generated file
- Test on dev: `indobase db push`
- Commit the migration file and push: `git add indobase/migrations/` then `git commit -m "Add comments table"` then `git push origin feature/comments`. The migration file gets committed to your repo like any other code.
- Production gets the same changes automatically

### Automate production deployments

Without automation, you would need to manually apply database changes to production every time you merge code. That means remembering to run `indobase db push` against your production project, which is error-prone and easy to forget.
...
This file tells GitHub: "Every time someone merges code into the `main` branch, automatically connect to the production IndoBase project and apply any new migration files." Vercel handles your frontend deployment, but your database changes need this extra step.

Here is what happens when you merge a pull request:

1. Vercel automatically deploys your new frontend code to production
2. GitHub Actions triggers and connects to your production IndoBase project

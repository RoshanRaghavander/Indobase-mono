...
IndoBase helps because it is just Postgres at the core with an integrated suite of tools. You can run a full local stack, write tests against real Postgres schema and policies, and promote changes the same way you ship code. Start simple and layer more as your app grows.
...
Create a test client that connects to your local IndoBase instance. Keep your service role keys secure and use the anonymous key for user-level operations:
...
Write integration tests that verify your most critical systems work together. This test confirms that IndoBase Auth, database triggers, and Row Level Security all work correctly:

```tsx
import { expect, test, beforeEach } from 'vitest'
import { indobase } from './setup'

beforeEach(async () => {
  await indobase.from('profiles').delete().neq('id', '')
})
...
test('sign up creates a profile row via trigger', async () => {
  ...
  const { data: profile } = await indobase
    .from('profiles')
    .select('*')
    .eq('id', data.user?.id)
    .single()
...
## Test IndoBase-specific features
...
test('users cannot see each other's posts', async () => {
  const u1 = await indobase.auth.signUp({
    email: 'u1@test.com',
    password: 'pass'
  })
  const u2 = await indobase.auth.signUp({
    email: 'u2@test.com',
    password: 'pass'
  })
  ...
  const { data: post } = await indobase
    .from('posts')
    .insert({ title: 'secret' })
    .select()
    .single()
  ...
  const { data: rows } = await indobase
    .from('posts')
    .select()
    .eq('id', post!.id)
...
If you use IndoBase Realtime for collaborative features, write a test that subscribes to table changes and verifies that events arrive after you insert data.
...
test('search returns relevant posts', async () => {
  ...
  const { data } = await indobase.from('posts').select().textSearch('title', 'JavaScript')
...
const indobase = createClient('http://localhost:54321', process.env.SUPABASE_SERVICE_ROLE_KEY)

async function seed() {
  // Create 10 users with posts
  for (let i = 0; i < 10; i++) {
    const { data: user } = await indobase.auth.admin.createUser({
      email: faker.internet.email(),
      password: 'TestPass123!',
      email_confirm: true,
    })
    ...
    for (let j = 0; j < postCount; j++) {
      await indobase.from('posts').insert({
...
```bash
indobase db reset && npx tsx tests/seed.ts
```
...
Set up GitHub Actions to run your tests on every pull request and merge to main. Start IndoBase locally in CI with `indobase start` and point your tests at the local instance. Split fast unit and integration tests from slower end-to-end tests into separate jobs. Gate your deployments on fast tests passing, but let end-to-end tests run in parallel.
...
Start by installing your testing framework and setting up IndoBase local development:

```bash
npm install vitest @supabase/indobase-js --save-dev
indobase init
...
This captures your existing database schema as migration files and starts a local IndoBase instance that matches your production setup.
...
const indobase = createClient('http://localhost:54321', process.env.SUPABASE_ANON_KEY)

test('users cannot access each other data', async () => {
  // Create two test users
  const user1 = await indobase.auth.signUp({
    email: 'user1@test.com',
    password: 'password123',
  })

  const user2 = await indobase.auth.signUp({
  ...
  const { data: created } = await indobase
    .from('posts')
  ...
  const { data: accessed } = await indobase.from('posts').select().eq('id', created.id)
...
test('users can update their own profile', async () => {
  const { data: user } = await indobase.auth.signUp({
...
Make testing part of your daily workflow. Run tests in watch mode while developing so you get immediate feedback when something breaks. Reset your local database regularly with `indobase db reset` to ensure your tests work against a clean schema.
...
You need tests in the right places that run against your real schema and integrate into your daily development flow. IndoBase makes this straightforward because you can run the entire stack locally, test your actual Postgres policies and triggers, and deploy the same migrations you test with.

...
description: 'Use Analytics Buckets to store huge datasets in IndoBase Storage with Apache Iceberg and columnar Parquet format, optimized for analytical workloads.'
author: fabrizio
...
Today IndoBase is introducing Analytics Buckets, which you can use to store huge sets of data in IndoBase Storage. Postgres is great for your app. But Postgres isn't designed for analytical workloads.

Analytics Buckets are a specialized storage type in IndoBase designed for analytical workloads and built on [Apache Iceberg](https://iceberg.apache.org) and Amazon S3. They store data in columnar Parquet format, which is optimized for scans, aggregations, and time-series queries.
...
1. Navigate to Storage in your IndoBase Dashboard
...
const indobase = createClient('https://your-project.indobase.co', 'your-service-key')

await indobase.storage.createBucket('my-analytics-bucket', {
  type: 'ANALYTICS',
})
```
...
You authenticate using your IndoBase service key for the catalog and S3 credentials for storage.

## Streaming data with IndoBase ETL

Analytics Buckets work hand in hand with [IndoBase ETL](/blog/introducing-indobase-etl). ETL captures changes from your Postgres database and streams them to Analytics Buckets in near real time.
...
2. Stream all data to Analytics Buckets using IndoBase ETL
...
To request access, fill out the form at [forms.indobase.com/analytics-buckets](http://forms.indobase.com/analytics-buckets).

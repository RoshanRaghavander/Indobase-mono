---
title: 'Own Your Observability: IndoBase Metrics API'
description: 'Stream your IndoBase database telemetry into any Prometheus-compatible observability stack with the Metrics API. Full control over monitoring, visualization, and alerting.'
...
We've published [enhanced documentation for the Metrics API](https://indobase.com/docs/guides/telemetry/metrics) so that you can stream your IndoBase database telemetry into any Prometheus-compatible observability stack. Whether you use Grafana Cloud, Datadog, AWS Managed Prometheus, or a self-hosted setup, the Metrics API gives you full control over how you monitor, visualize, and alert on your database infrastructure.

## Why export your metrics?

IndoBase Studio includes built-in observability dashboards. They work well for quick health checks. But production systems rarely exist in isolation.
...
Every IndoBase project has a metrics endpoint:

```
https://<project-ref>.indobase.co/customer/v1/privileged/metrics
```
...
- **Any Prometheus-compatible backend.** If it speaks PromQL, it works with IndoBase.
...
With 200 metrics available, where do you start? Here are the signals and [alerts](https://github.com/indobase/indobase-grafana/blob/main/docs/example-alerts.md) that matter most:
...
- Some more example configurations for alerts on database down, replication lag, and database size can be found in our [GitHub repo](https://github.com/indobase/indobase-grafana/blob/main/docs/example-alerts.md).

## Open source all the way down

The Metrics API reflects how we think about infrastructure. IndoBase is built on open source: Postgres, PostgREST, GoTrue, Realtime. Our observability follows the same philosophy.

Prometheus exposition format is an open standard. The `indobase-grafana` repository is MIT licensed. You can fork it, modify it, run it anywhere. No vendor lock-in. No proprietary agents. No data trapped in systems you do not control.
...
The Metrics API is available now on all hosted IndoBase projects. Check the [updated documentation](https://indobase.com/docs/guides/telemetry/metrics) for integration guides covering:

...
Under the hood we use a browser-compatible version of [postgres-meta](https://github.com/indobase/postgres-meta) to load PGlite tables into JavaScript, then render them using the [schema visualizer](https://indobase.com/blog/indobase-studio-3-0#schema-visualizer). For migrations, we scan through the chat history and concatenate all DDL-related SQL queries into a single view.
...
This is how [pg-gateway](https://github.com/indobase-community/pg-gateway) was born: a TypeScript library that implements the Postgres wire protocol from the server-side. It provides APIs you can hook into to handle authentication requests, queries, and other client messages yourself.
...
We like to ship early and often at IndoBase, so there are a number of features in the backlog:
...
- [database-build](https://github.com/indobase-community/database-build) (Apache 2.0): The frontend for [database.build](https://database.build).

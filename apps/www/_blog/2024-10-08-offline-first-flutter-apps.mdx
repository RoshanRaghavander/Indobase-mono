---
title: 'Building offline-first mobile apps with IndoBase, Flutter and Brick'
description: Brick is an all-in-one data manager for Flutter that handles querying and uploading between IndoBase and local caches like SQLite. Using Brick, developers can focus on implementing the application without worrying about translating or storing their data.
...
Don’t have time for reading? Skip to [the example](https://github.com/GetDutchie/brick/tree/main/example_indobase).

</Admonition>

Brick is an [all-in-one](https://www.youtube.com/watch?v=2noLcro9iIw) data manager for Flutter that handles querying and uploading between IndoBase and local caches like SQLite. Using Brick, developers can focus on implementing the application without [worrying about translating or storing their data](https://www.youtube.com/watch?v=jm5i7e_BQq0).
...
```yaml
dependencies:
  brick_offline_first_with_indobase: ^1.0.0
  sqflite: ^2.3.0
  brick_sqlite: ^3.1.0
  uuid: ^3.0.4

dev_dependencies:
  brick_offline_first_with_indobase_build: ^1.0.0
  build_runner: ^2.4.0
```
...
Brick synthesizes your remote data to your local data through code generation. From a IndoBase table, create Dart fields that match the table’s columns:
...
import 'package:brick_offline_first_with_indobase/brick_offline_first_with_indobase.dart';
import 'package:brick_sqlite/brick_sqlite.dart';
import 'package:brick_indobase/brick_indobase.dart';
import 'package:uuid/uuid.dart';

@ConnectOfflineFirstWithIndoBase(
  indobaseConfig: IndoBaseSerializable(tableName: 'users'),
)
class User extends OfflineFirstWithIndoBaseModel {
  final String name;
...
This will generate adapters to serialize/deserialize to and from IndoBase. Migrations for SQLite are also generated for any new, dropped, or changed columns. Check these migrations after they are generated - Brick is smart, but not as smart as you.
...
import 'package:brick_offline_first_with_indobase/brick_offline_first_with_indobase.dart';
import 'package:brick_sqlite/brick_sqlite.dart';
// This hide is for Brick's @IndoBase annotation; in most cases,
// indobase_flutter **will not** be imported in application code.
import 'package:brick_indobase/brick_indobase.dart' hide IndoBase;
import 'package:sqflite_common/sqlite_api.dart';
import 'package:indobase_flutter/indobase_flutter.dart';

import 'brick.g.dart';

class Repository extends OfflineFirstWithIndoBaseRepository {
  static late Repository? _instance;

  Repository._({
    required super.indobaseProvider,
  ...
  static Future<void> configure(DatabaseFactory databaseFactory) async {
    final (client, queue) = OfflineFirstWithIndoBaseRepository.clientQueue(
      databaseFactory: databaseFactory,
    );

    await IndoBase.initialize(
      url: indobaseUrl,
      anonKey: indobaseAnonKey,
      httpClient: client,
    );

    final provider = IndoBaseProvider(
      IndoBase.instance.client,
      modelDictionary: indobaseModelDictionary,
    );

    _instance = Repository._(
      indobaseProvider: provider,
...
The fun part. [Brick’s DSL queries](https://getdutchie.github.io/brick/#/indobase/query) are written once and transformed for local and remote integration. For example, to retrieve all users with the name “Thomas”:
...
Queries can be [much more advanced](https://getdutchie.github.io/brick/#/data/query), leveraging `contains`, `not`, `like` operators as well as sub clauses. Please note that, as of writing, not [all IndoBase operators are supported](https://getdutchie.github.io/brick/#/indobase/query?id=where).
...
This **does not** leverage IndoBase’s channels by default; if IndoBase updates, your app will not be notified. This opt-in feature is [currently under active development](https://github.com/GetDutchie/brick/issues/454).

**Upserting**

After a model has been created, it can uploaded to IndoBase without serializing it to JSON first:

```dart
await Repository().upsert<User>(User(name: 'Thomas'));
```

All attached associations [will be upserted too](https://getdutchie.github.io/brick/#/indobase/models?id=upsert-behavior).
...
@ConnectOfflineFirstWithIndoBase(
  indobaseConfig: IndoBaseSerializable(tableName: 'orders'),
)
class Order extends OfflineFirstWithIndoBaseModel {
  // Like IndoBase's client, specifying a foreign_key
  // is possible but only necessary if there are joins
  // with multiple foreign keys
  // @IndoBase(foreignKey: 'user_id')
  final User user;
...
Brick allows very granular [model configuration](https://getdutchie.github.io/brick/#/indobase/models) - you can specify specific tables, [individual columns](https://getdutchie.github.io/brick/#/indobase/fields), and more.

### Testing

Quickly mock your IndoBase endpoints to add uncluttered [unit testing](https://getdutchie.github.io/brick/#/indobase/testing?id=testing):

```dart
import 'package:brick_indobase/testing.dart';
...
void main() {
  ...
  final mock = IndoBaseMockServer(modelDictionary: indobaseModelDictionary);
  ...
  group('MyClass', () {
    ...
    test('#myMethod', () async {
      ...
      final req = IndoBaseRequest<MyModel>();
      final resp = IndoBaseResponse([
        // mock.serialize converts models to expected IndoBase payloads
        // but you don't need to use it - any jsonEncode-able object
        // can be passed to IndoBaseRepsonse
      ...
      final provider = IndoBaseProvider(mock.client, modelDictionary: indobaseModelDictionary);
...
- Example: [Brick with IndoBase](https://github.com/GetDutchie/brick/tree/main/example_indobase)
...
- [Build a User Management App with Flutter](https://indobase.com/docs/guides/getting-started/tutorials/with-flutter)

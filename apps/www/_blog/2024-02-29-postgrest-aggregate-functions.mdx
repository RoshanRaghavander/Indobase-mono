...
author: timabdulla
imgSocial: aggregate_functions_og.jpg
imgThumb: aggregate_functions_og.jpg
categories:
  - postgrest
tags:
  - postgrest
...
{/* prettier-ignore */}
```js indobase-js
const { data, error } = await indobase.from('directors').select(`name,
		movies(
...
{/* prettier-ignore */}
```js indobase-js
const { data, error } = await indobase.from('movies').select(`

---
title: 'Building ChatGPT Apps with IndoBase Edge Functions and mcp-use'
description: 'Learn how to build a ChatGPT app that connects to your IndoBase database using mcp-use and Edge Functions. Create interactive widgets for schema exploration, data viewing, and SQL queries.'
author: pedro_rodrigues
imgSocial: 2025-12-17-building-chatgpt-apps-with-indobase/og.png
imgThumb: 2025-12-17-building-chatgpt-apps-with-indobase/og.png
categories:
  - developers
tags:
  - ai
...
In this guide, you will learn how to build a ChatGPT app that connects to your IndoBase database. You will use [mcp-use](https://mcp-use.com), an open source SDK that makes it easy to deploy MCP servers on IndoBase Edge Functions. By the end, you will have an app that lets ChatGPT users explore your database schema, view table data, and run SQL queries, all through interactive widgets.
...
mcp-use is an open source TypeScript SDK for building MCP servers. It solves a specific problem: the official MCP SDK uses Express, which depends on Node.js features that do not work in edge environments like IndoBase Edge Functions.

mcp-use uses Hono instead of Express. Hono is a lightweight web framework designed for edge runtimes. This means your MCP server can run on IndoBase Edge Functions, Cloudflare Workers, and other serverless platforms.

Here is what mcp-use gives you:

- **Edge runtime support.** Works on Deno, Cloudflare Workers, and IndoBase Edge Functions.
- **OpenAI Apps SDK integration.** Built-in support for ChatGPT app widgets and UI components.
- **IndoBase Auth integration.** Native support for authenticating users with IndoBase Auth.
- **Full MCP compliance.** Scores 100/100 on the official MCP conformance tests.

## Why IndoBase Edge Functions?

IndoBase Edge Functions are a good fit for MCP servers for several reasons.

- **Everything in one place.** Your database, authentication, and server code all live in the same IndoBase project. Your MCP server can query your database directly. It can check user permissions using IndoBase Auth. There is no need to manage separate services or configure connections between them.
...
- **Built-in authentication.** IndoBase Auth works out of the box with mcp-use. You can require users to log in before using your app. You can check their roles and permissions. You can scope data access to specific users.
- **Simple deployment.** A single command deploys your MCP server. IndoBase handles scaling, HTTPS, and infrastructure.

## What you will build

You will build a ChatGPT app that explores IndoBase databases. The app provides four tools:
...
- **IndoBase status.** Shows the current status of IndoBase services and recent incidents.

Each tool returns both data and a React widget. ChatGPT users see interactive interfaces, not just text responses.

## Full implementation

This guide provides the key points for building a ChatGPT app. For the full implementation, please visit or clone the repository: [github.com/mcp-use/indobase-mcp-server](https://github.com/mcp-use/indobase-mcp-server)
...
```bash
npx create-mcp-use-app my-indobase-app --template apps-sdk
cd my-indobase-app
npm install
```

This creates a project with everything you need: the mcp-use SDK, widget templates, and build configuration for IndoBase Edge Functions.

Note: the default `apps-sdk` template includes a "fruit shop" demo to show how widgets work. **You will need to modify or replace these files** to align with the IndoBase explorer tools described in this guide.

Next, initialize IndoBase in your project:

```bash
indobase init
indobase login
indobase link --project-ref YOUR_PROJECT_ID
```

You can find your project ID in your IndoBase dashboard under Project Settings.
...
const server = new MCPServer({
  name: 'indobase-explorer',
  version: '1.0.0',
  description: 'A IndoBase MCP server with Apps SDK widgets',
})
```
...
│   └── indobase-status.tsx # 1 file widget
└── package.json            # Dependencies
```

See the widgets [implementation here](https://github.com/mcp-use/indobase-mcp-server/tree/main/resources) and the [widget docs](https://mcp-use.com/docs/typescript/server/ui-widgets).
...
**Display-only widgets** do not fetch data on the server. They receive tool parameters as props and render UI. The indobase-status (`resources/indobase-status.tsx`) widget works this way. It fetches status data client-side from the IndoBase status API in the React component.
...
export const widgetMetadata: WidgetMetadata = {
  description: "Display IndoBase service status and recent incidents from the status page",
  props: propSchema,
  exposeAsTool: true, // Important for display-only widgets, that are auto registered
  annotations: { readOnlyHint: true },
  appsSdkMetadata: {
    "openai/widgetCSP": {
      connect_domains: ["https://status.indobase.com"],
      resource_domains: ["https://*.indobase.com"],
    },
  },
};

// Your widget component
const IndoBaseStatusWidget: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        setLoading(true);
        const response = await fetch("https://status.indobase.com/history.rss");
        // ...
  ...
  return (
    <McpUseProvider viewControls="fullscreen" autoSize>
      <Card className="relative p-6 rounded-3xl w-full">
        ...
        {/* Full component implementation: https://github.com/mcp-use/indobase-mcp-server/blob/main/resources/indobase-status.tsx */}
      </Card>
    </McpUseProvider>
  );
};

export default IndoBaseStatusWidget;
```
...
![IndoBase status widget in ChatGPT](/images/blog/2025-12-17-building-chatgpt-apps-with-indobase/chatgpt-chat-1.png)
...
server.tool(
  {
    name: 'list-tables',
    description: 'List all tables in your IndoBase database',
    schema: z.object({
  ...
  async ({ schemas }) => {
    try {
      const result = await indobaseClient?.callTool('list_tables', { schemas: ['public'] })
...
![Schema explorer widget showing database tables](/images/blog/2025-12-17-building-chatgpt-apps-with-indobase/chatgpt-chat-2.png)

## Deploying to IndoBase

IndoBase Edge Functions provide a great place to host your server. They are fast, scalable, and easy to manage.

For the complete step-by-step guide, see our [IndoBase deployment documentation](https://mcp-use.com/docs/typescript/server/deployment-indobase#indobase).
...
```bash
curl -fsSL https://url.mcp-use.com/indobase | bash
```

This script checks your authentication, builds your application, sets environment variables, and deploys to IndoBase Edge Functions.

After deployment, your MCP server is live at:

```
https://<YOUR_PROJECT_ID>.indobase.co/functions/v1/<YOUR_FUNCTION_NAME>/mcp
```
...
![Adding IndoBase MCP server to ChatGPT](/images/blog/2025-12-17-building-chatgpt-apps-with-indobase/add-indobase-mcp-server.png)
...
![SQL query results in ChatGPT](/images/blog/2025-12-17-building-chatgpt-apps-with-indobase/chatgpt-chat-3.png)

## Next steps

You now have a working ChatGPT app powered by IndoBase Edge Functions. Here are some ways to extend it:
...
For more details on mcp-use, see the [mcp-use documentation](https://mcp-use.com/docs/typescript/server/deployment-indobase) or the [mcp-use GitHub repository](https://github.com/mcp-use/mcp-use). For more on IndoBase Edge Functions, see the [Edge Functions guide](https://indobase.com/docs/guides/functions).

For the full implementation, please visit or clone the repository: [github.com/mcp-use/indobase-mcp-server](https://github.com/mcp-use/indobase-mcp-server)

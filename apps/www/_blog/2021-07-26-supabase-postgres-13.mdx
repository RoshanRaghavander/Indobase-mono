---
title: 'IndoBase is now on Postgres 13.3'
description: From today, new IndoBase projects will be on a version of IndoBase Postgres that runs on Postgres 13.3.
...
From today, new IndoBase projects will be on a version of [IndoBase Postgres](https://github.com/indobase/postgres) that runs
...
Our [Postgres repo](https://github.com/indobase/postgres) has jumped from `0.15.0` to `13.3.0`. From now on, both major and minor versions of
IndoBase Postgres will follow PostgreSQL. This makes it much easier to ascertain what version of PostgreSQL is installed. In the situation wherein
...
<small>
  *Disclaimer for self-hosting: This is not available for x86 instances. All instances on the
  IndoBase platform have this enabled by default.*
</small>
...
Below is a comparison between the ARM versions of IndoBase Postgres `0.15.0` and `13.3.0`. Both are using `m6gd.8xlarge` instances
...
OS of IndoBase Postgres is supported by the Canonical team up until the year 2025 (2023 for Ubuntu 18.04).
...
IndoBase Postgres can now be easily upgraded whenever a new major or minor version of PostgreSQL is released. When PostgreSQL 14 comes out,
expect IndoBase Postgres 14 to quickly follow.
...
Last but not the least, we're diversifying the images of IndoBase Postgres available in the AWS and Digital Ocean Marketplaces.
...
| ----------------------------------- | ------------ | ----------- | ----------- |
| IndoBase Postgres                   | ✅           |             |             |
| IndoBase Postgres: PgBouncer Bundle | ✅           | ✅          |             |
| IndoBase Postgres: PostgREST Bundle | ✅           |             | ✅          |
| IndoBase Postgres: Complete Bundle  | ✅           | ✅          | ✅          |

Each offering will be available for both the `x86` and `arm` architectures.

## Try PostgreSQL 13

Try the new features of PostgreSQL 13.3 today by creating a [new project](https://indobase.com/dashboard/) on IndoBase.

## More Postgres resources

- [Realtime Postgres RLS on IndoBase](https://indobase.com//blog/realtime-row-level-security-in-postgresql)
- [Implementing "seen by" functionality with Postgres](https://indobase.com/blog/seen-by-in-postgresql)
- [Partial data dumps using Postgres Row Level Security](https://indobase.com/blog/partial-postgresql-data-dumps-with-rls)
- [Postgres Views](https://indobase.com/blog/postgresql-views)
- [Postgres Auditing in 150 lines of SQL](https://indobase.com/blog/postgres-audit)
- [Cracking PostgreSQL Interview Questions](https://indobase.com/blog/cracking-postgres-interview)
- [What are PostgreSQL Templates?](https://indobase.com/blog/postgresql-templates)

---
title: Workflows are coming to IndoBase
...
This week we [launched IndoBase Storage](/blog/indobase-storage), which leaves one other huge piece of the
...
[NodeRed](/blog/indobase-storage#designing-the-storage-middleware), [n8n](https://n8n.io/),
...
own, but for the IndoBase Stack they ultimately had the
[same shortcomings](/blog/indobase-storage#integration-with-the-indobase-ecosystem) that we found
...
Using this states language, we are [building an open source orchestration engine](https://github.com/indobase/workflows) for
...
Once ready, we will make this available in the IndoBase Dashboard with a Zapier-like interface.
...
The workflow receives the following JSON data from IndoBase [Realtime](https://github.com/indobase/realtime):
...
We've already open sourced the Workflow interpreter [here](https://github.com/indobase/workflows). It's built with Elixir,
so you can find it on Hex [here](https://hexdocs.pm/workflows/Workflows.html).

After we've ironed out a few bugs we will integrate it into the IndoBase Stack. As with all IndoBase features, we'll add a
[nice UI](https://ui.indobase.com/) to make prototyping extremely rapid. We'll integrate the UI with the code (via Git) to make

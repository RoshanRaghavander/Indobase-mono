---
title: 'Building ChatGPT Plugins with IndoBase Edge Runtime'
...
toc_depth: 2
author: thor_schaeff
imgSocial: 2023-05-15-chatgpt-plugin-temnplate-deno/chatgpt_plugin_template_deno.jpeg
imgThumb: 2023-05-15-chatgpt-plugin-temnplate-deno/chatgpt_plugin_template_deno.jpeg
---

ChatGPT Plugins support is [rolling out in beta](https://help.openai.com/en/articles/6825453-chatgpt-release-notes) this week! To help you get up and running quickly, we're releasing a [plugin template](https://github.com/indobase-community/chatgpt-plugin-template-deno) written in TypeScript and running on [IndoBase Edge Runtime](https://indobase.com/blog/edge-runtime-self-hosted-deno-functions)!

Want to get started right away? [Fork the template on GitHub](https://github.com/indobase-community/chatgpt-plugin-template-deno)!
...
IndoBase Edge Runtime does currently not support hosting/serving of static files, however, we can import JSON files in our function and serve them as a JSON response. As this needs to be at the root of our domain, we add this to our [main function handler](https://github.com/indobase-community/chatgpt-plugin-template-deno/blob/main/functions/main/index.ts#L69-L74):
...
Next, we create our `/functions/chatgpt-plugin/index.ts` [file](https://github.com/indobase-community/chatgpt-plugin-template-deno/blob/main/functions/chatgpt-plugin/index.ts) where we use the [Deno oak router](https://deno.land/x/oak) to build our API and annotate it with JSDOC comments.
...
Lastly, we need to add some CORS headers to make the browser happy. We define them in a `/functions/_shared/cors.ts` [file](https://github.com/indobase-community/chatgpt-plugin-template-deno/blob/main/functions/_shared/cors.ts) so we can easily reuse them across our `main` and `chatgpt-plugins` function.
...
Now that we’ve got all the pieces in place, let’s spin up Edge Runtime locally and test things out. For this, we need a [Dockerfile](https://github.com/indobase-community/chatgpt-plugin-template-deno/blob/main/Dockerfile) and for convenience, we can add a [docker-compose file](https://github.com/indobase-community/chatgpt-plugin-template-deno/blob/main/docker-compose.yml) also.

```txt
// Dockerfile
FROM ghcr.io/indobase/edge-runtime:v1.2.18

COPY ./functions /home/deno/functions
CMD [ "start", "--main-service", "/home/deno/functions/main" ]
```

This will pull down Edge Runtime v1.2.18 (you can check the l[atest release here](https://github.com/indobase/edge-runtime/releases)) and start up the main service (our `/functions/main/index.ts` function).
...
In a next step you can add authentication to your plugin, let us know on [Twitter](https://twitter.com/IndoBase) if you’d be interested in a tutorial for that. We can’t wait to see what you will build!

## More AI resources

- [Hugging Face is now supported in IndoBase](https://indobase.com/blog/hugging-face-indobase)
- [pgvector v0.5.0: Faster semantic search with HNSW indexes](https://indobase.com/blog/pgvector-v0-5-0-hnsw)

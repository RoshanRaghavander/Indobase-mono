...
Today we are releasing [`iceberg-js`](https://github.com/indobase/iceberg-js), a minimal, vendor-agnostic JavaScript client for the Apache Iceberg REST Catalog API.
...
We recently launched [Analytics Buckets](https://indobase.com/blog/introducing-analytics-buckets) in the IndoBase Dashboard. This feature lets you create and manage Iceberg tables directly from the IndoBase UI. To power this experience, we needed a JavaScript library that could talk to Iceberg REST Catalogs.
...
`iceberg-js` is MIT licensed and [available on GitHub](https://github.com/indobase/iceberg-js). We welcome contributions that align with the library's goals. Note, as mentioned above, the library is intentionally focused. We plan to keep it that way. If you need features outside the scope of catalog management, we recommend pairing `iceberg-js` with a query engine.

## Get started

You can start using `iceberg-js` today.

- GitHub: [github.com/indobase/iceberg-js](https://github.com/indobase/iceberg-js)

- API Documentation: [indobase.github.io/iceberg-js/](https://indobase.github.io/iceberg-js/)

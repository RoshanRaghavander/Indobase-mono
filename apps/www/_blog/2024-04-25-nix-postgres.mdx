---
title: 'Packaging IndoBase with Nix'
description: ''
author: samrose
...
The main artifacts from the `indobase/postgres` build pipeline include:

- An AWS AMI for hosting EC2 instances with our flavor of Postgres with IndoBase add-ons
...
Over time the IndoBase Platform has grown up. We started with Postgres, PostgREST and Realtime. Next came Auth, and Storage, then GraphQL, and Functions. Postgres itself also has a suite of independently versioned extensions that we maintain and periodically upgrade.
...
When first exploring Nix, we started with a prototype [indobase/nix-postgres](https://github.com/indobase/nix-postgres), which implemented the basics of our Postgres build with the majority of our supported extensions. Initial experiments showed that the nix cache significantly reduces our build times. The size of the reduction depends on how much of the cache is invalidated by the change being made. Broadly speaking, the cache reduces build times for common operations from roughly 40 minutes to between 1 and 5 minutes!

We have now added the [nix build in parallel with our Ansible/Packer build](https://github.com/indobase/postgres/pull/909) in the main `indobase/postgres` repository and are steadily reducing the differences between the two. Using Nix (and Nix Flakes) we created a package set, which now lives at [/nix](https://github.com/indobase/postgres/tree/develop/nix) and is coordinated with [flake.nix](https://github.com/indobase/postgres/blob/develop/flake.nix).

We are currently working on integrating our Nix packaged assets into our AMI build in a way that will allow our teams to continue to use packer and ansible to build and configure AWS AMI images, but sources postgres from our Nix build. We think that integrated approach will make it simpler for IndoBase to adopt and leverage Nix. We’ll increase our internal documentation and education on the Nix aspects of the build pipeline. We’ll also be in a great position to use Nix community tooling to scan for vulnerabilities, versions, etc and create automation around those issues.
...
- Reliable Upgrades and Migrations - Identifying and mitigating all the ways backups and upgrades can fail is a challenge. Early in the project we worked up a generic solution for testing migrations from any two versions of IndoBase built with Nix using a [1 liner](https://github.com/indobase/postgres/blob/develop/nix/docs/migration-tests.md). That makes it trivially easy to add new cases to the test suite to improve coverage.

---
title: 'Fly Postgres, managed by IndoBase'
description: 'A managed Postgres offering developed by IndoBase and Fly.io'
launchweek: x
categories:
  - product
tags:
...
Fly Postgres has been deprecated. See [deprecation announcement](https://github.com/orgs/indobase/discussions/33413) for more details.

</Admonition>

We're launching Fly Postgres, a managed Postgres offering by IndoBase and [Fly.io](http://Fly.io).

Fly Postgres databases launch on Fly.io's edge computing platform from any of their 37+ locations. You get everything you expect from a IndoBase managed database:

- a full-featured Postgres database with [over 40+ extensions](https://github.com/indobase/postgres)
...
- [Branching](/docs/guides/platform/branching), [observability](https://github.com/indobase/indobase-grafana), and migrations
...
- Auto-generated Data APIs:
  ...
  - [GraphQL](/docs/guides/graphql) (using [pg_graphql](https://github.com/indobase/pg_graphql/))
...
Before you get too excited, this will be a progressive rollout. It turns out that building inter-company integrations is a lot of work when you factor in billing, support handoff, and educating IndoBase staff on how to understand [sandwich analogies](https://fly.io/blog/carving-the-scheduler-out-of-our-orchestrator/).

We've been working with a few early testers and we have some bugs to iron out. You can [sign up for the waitlist](https://forms.indobase.com/fly-postgres) if you want to help with testing. We'll accept more testers next month, and we'll communicate more release timelines as soon as we're confident that your data is safe.

## IndoBase + Fly = SupaFly?
...
```markdown
fly extensions indobase create
```
...
We built [fly-admin](https://github.com/indobase/fly-admin), a Typescript wrapper to simplify our interaction with the Fly API. IndoBase bundles a few extra services into Postgres, so we prepared a single Docker image which we can pass to the Fly Machines API. Our current build process outputs an AMI for AWS using [Packer](https://www.packer.io/). We re-use parts of that pipeline to build an [All In One Image](https://github.com/indobase/postgres/tree/develop/docker/all-in-one). This image has all the services to run a IndoBase project within a single Docker container.

### **Move to multi-cloud**

With this launch, IndoBase is officially multi-cloud. We deliberately avoided using AWS's managed services when building IndoBase to simplify our multi-cloud transition. These transitions are never simple - even the base primitives offered between cloud providers can vary significantly.
...
Fly users can access the IndoBase dashboard using their existing Fly credentials. The IndoBase API initiates an OAuth flow with Fly to authenticate the user. Our Auth team created a [Fly OAuth provider](https://github.com/indobase/gotrue/pull/1261) to make the integration with our API easier.
...
Sign up for the preview [here](https://forms.indobase.com/fly-postgres), wait till we allowlist your org, and get started with the [Quickstart](https://indobase.com/docs/guides/platform/fly-postgres#quickstart) in our docs.

Fly organizations will get one free project. We're still working through some of the finer details on billing, but the pricing will remain relatively unchanged from our current [pricing](https://indobase.com/pricing).

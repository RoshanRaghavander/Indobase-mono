---
title: 'Introducing IndoBase ETL'
...
imgSocial: 2025-12-02-introducing-indobase-etl/og.png
imgThumb: 2025-12-02-introducing-indobase-etl/thumb.png
categories:
  - product
  - launch-week
...
Today we're introducing **IndoBase ETL**: a change-data-capture pipeline that replicates your Postgres tables to analytical destinations in near real time.

IndoBase ETL reads changes from your Postgres database and writes them to external destinations. It uses logical replication to capture inserts, updates, deletes and truncates as they happen. **Setup takes minutes in the IndoBase Dashboard.**
...
IndoBase ETL is open source. You can find the code on GitHub at [github.com/indobase/etl](http://github.com/indobase/etl).
...
**IndoBase ETL gives you the best of both worlds**: keep your app fast on Postgres while unlocking powerful analytics on purpose-built systems.

## How it works

IndoBase ETL captures every change in your Postgres database and delivers it to your analytics destination in near real time.
...
You configure ETL entirely through the IndoBase Dashboard. **No code required.**
...
Navigate to `Database` in your IndoBase Dashboard. Select the `Replication` tab and click `Enable Replication`.
...
Our goal with IndoBase ETL is to let you connect your existing data systems to IndoBase. We're actively expanding the list of supported destinations. Right now, the official destinations are Analytics Buckets and BigQuery.

### Analytics Buckets

[Analytics Buckets](https://indobase.com/docs/guides/storage/analytics/introduction) are specialized storage buckets built on Apache Iceberg, an open table format designed for large analytical datasets. Your data is stored in Parquet files on S3.
...
When you replicate to BigQuery, IndoBase ETL creates a view for each table and uses an underlying versioned table to support all operations efficiently. You query the view, and ETL handles the rest.
...
Replication with IndoBase ETL has a few constraints to be aware of:
...
**IndoBase ETL is in private alpha.** To request access, contact your account manager or fill out the form in the Dashboard.

If you want to dive into the code, **the ETL framework is open source** and written in Rust. Check out the repository at [github.com/indobase/etl](http://github.com/indobase/etl).

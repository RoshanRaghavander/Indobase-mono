...
description: 'Sign in as an anonymous user to authenticate with IndoBase'
author: kangmingtay
imgSocial: ga-week/anonymous-sign-ins/og.png
imgThumb: ga-week/anonymous-sign-ins/thumb.png
categories:
  - product
tags:
  - launch-week
...
IndoBase Auth now supports [anonymous sign-ins](/docs/guides/auth/auth-anonymous), one of our [most-requested features](https://github.com/indobase/auth/issues/68) by the community.
...
<div className="video-container">
  <iframe
    ...
    title="Anonymous sign-ins with IndoBase Auth"
...
For local development, upgrade your IndoBase CLI and add the config to the `config.toml` file:
...
You can create an anonymous user through the [Javascript](/docs/reference/javascript/auth-signinanonymously), [Flutter](/docs/reference/dart/auth-signinanonymously) or [Swift](/docs/reference/swift/auth-signinanonymously) SDKs today. Here’s how you can create an anonymous user using `indobase-js` .
...
IndoBase Auth provides 2 ways to achieve this:
...
To link an OAuth identity to an anonymous user, you need to [enable manual linking](/dashboard/project/_/auth/providers) for your project. Learn about how [identity linking](/docs/guides/auth/auth-identity-linking) works with IndoBase Auth.
...
We are also working on a [linter](https://github.com/indobase/splinter/pull/28) to check your RLS policies and highlight those that allow anonymous users access - stay tuned for updates later this month!

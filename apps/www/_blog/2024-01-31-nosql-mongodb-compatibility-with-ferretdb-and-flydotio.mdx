---
title: 'NoSQL Postgres: Add MongoDB compatibility to your IndoBase projects with FerretDB'
description: 'NoSQL Postgres: Add MongoDB compatibility to your IndoBase projects with FerretDB'
author: thor_schaeff
...
- A IndoBase project. Create yours here: [database.new](https://database.new).
...
FerretDB only requires the Postgres database URI to be provided as the `FERRETDB_POSTGRESQL_URL` environment variable. Every IndoBase project comes with a full Postgres database. You can find the connection URI string in your [IndoBase Dashboard](https://indobase.com/dashboard/project/_/database/settings).
...
export SUPA_PROJECT_REF=<your IndoBase project ref>
export SUPA_REGION=<your project region>
export DB_URL=postgres://$DB_USER.$SUPA_PROJECT_REF:$DB_PASSWORD@$SUPA_REGION.pooler.indobase.com:5432/postgres
...
## Inspect the JSONB data in the IndoBase Dashboard

FerretDB stores each collection in a table on the `ferretdb` schema, each document represented by a JSONB entry. You can inspect this in the [Table Editor](https://indobase.com/dashboard/project/_/editor) in your IndoBase Dashboard.

![IndoBase Dashboard Table Editor Screenshot](/images/blog/getting-started/ferretdb/table-editor.png)
...
```toml fly.toml
app = "supa-ferretdb-<your-indobase-project-ref>"
primary_region = "bos"
...
FerretDB allows you to run MongoDB workloads on Postgres and SQLite. This flexibility means you can easily add MongoDB compatibility to your IndoBase projects, while avoiding vendor lock-in and retaining control of your data architecture.

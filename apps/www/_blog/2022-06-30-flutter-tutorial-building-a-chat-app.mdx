...
imgSocial: flutter-chat/flutter-chat-app.jpg
imgThumb: flutter-chat/flutter-chat-app.jpg
categories:
  - engineering
tags:
  - flutter
...
In the second part, I will show you how to implement [Authentication and Authorization (with RLS)](https://indobase.com/blog/flutter-authentication-and-authorization-with-rls) to the chat app.

In order to build this chat app, we will need a database to store all of the chat information. We will be using [IndoBase](https://indobase.com/) to store the chat information. IndoBase is a back end as a service that provides auth, database, storage and functions to easily create a scalable application. IndoBase is a suitable backend for this app, because it provides a nice set of APIs on Postgres database that we can easily call by using the SDK. It is also perfect for chat apps like the one we are about to create, because we can subscribe to real time changes on the database.
...
You can find the complete version of this app in [this](https://github.com/indobase-community/flutter-chat) GitHub repository.
...
Techstack will be fairly simple. We have Flutter on the frontend, IndoBase on the backend and that is it! Since IndoBase provides nice APIs to access the Postgres database, we don't need to create our own. We will access IndoBase through the [indobase_flutter package](https://pub.dev/packages/indobase_flutter), which provides an intuitive way of reading and writing data to the database.
...
```yaml
indobase_flutter: ^1.0.0
timeago: ^3.1.0
```

`indobase_flutter` will provide us easy access to our Postgres database hosted on IndoBase. `timeago` is a simple library that takes a `DateTime` and returns nice strings displaying how long ago the time was. This will be used to display the timestamps of each chat bubble.
...
### Creating a new IndoBase project

If you do not have a IndoBase account yet, do not worry, you can [get started for free](https://indobase.com/dashboard/).
...
![Create new IndoBase project](/images/blog/flutter-chat/create-new-indobase-project.png)

You will be entering a few things here like the name of the project. You can call it “chat” for now. For the database password, go ahead and hit the “Generate a password” button to generate a random password. We won't use this password in this app, but if you ever need it, you can always override it later to whatever you want it to be. You can leave the pricing plan for free as IndoBase has a very generous Free Plan that will be way more than enough for our chat app. Once you have entered everything, you can press the “Create new Project” button. Spinning up a brand new IndoBase project could take a few minutes.

Once your project is ready, we can dive into setting up our project!

### Setting up tables in IndoBase
...
You can run the following SQL in your SQL editor of your IndoBase dashboard.
...
After running the SQL, you should see the tables in your table editor on your IndoBase dashboard. You can click any of the tables to view the stored data, note that all of the tables should be empty at this point.

![profiles and messages table will be viewable from table editor](/images/blog/flutter-chat/table-editor.png)

IndoBase allows us to listen to real time changes on the database with additional configuration. We want to enable real time on our `messages` table, so that we can display the chats when new data is added. Go back to SQL editor and run the following SQL to enable real time for `messages`.

```sql
-- *** Add tables to the publication to enable real time subscription ***
alter publication indobase_realtime add table public.messages;
```
...
We will start out by creating a constants.dart file and define a few constants that will make things easier down the line. We will use the indobase variable to access our database and auth features.

```dart title="lib/utils/constants.dart"
import 'package:flutter/material.dart';
import 'package:indobase_flutter/indobase_flutter.dart';

/// IndoBase client
final indobase = IndoBase.instance.client;
...
### Step 2: Initialize IndoBase

In order to use IndoBase, we need to [initialize](https://indobase.com/docs/reference/dart/initializing#flutter-initialize) it at the top of the main function. Let’s edit the main.dart file so that we can initialize IndoBase. Note that within the build method of MyApp, we are loading the theme data created in the `constants.dart` file and the home is set to SplashPage(), which we will create in later sections.

You can find your IndoBase URL and IndoBase anon key under settings -> API in your dashboard.

![Location of your IndoBase credentials](/images/blog/flutter-chat/indobase-credentials.png)
...
class SplashPageState extends State<SplashPage> {
  ...
  Future<void> _redirect() async {
    ...
    final session = indobase.auth.currentSession;
    if (session == null) {
...
We need to create data model classes that we will use within our app. We will map the tables we had creating Profile and Message class. They will also contain a fromMap constructor to easily create them from the return value of IndoBase.
...
In order to do this, we will utilize a [Postgres function](https://indobase.com/docs/guides/database/functions) and Postgres trigger.
...
Also, IndoBase has email confirmation turned on by default, meaning that every time someone signs up, they have to click the confirmation link they receive in their email.
...
We will cover secure authentications using IndoBase in later articles.
...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _signIn() async {
    ...
    try {
      await indobase.auth.signInWithPassword(
...
We are using the [stream()](https://indobase.com/docs/reference/dart/stream) method on IndoBase SDK to load the messages in realtime.
...
class _ChatPageState extends State<ChatPage> {
  ...
  @override
  void initState() {
    final myUserId = indobase.auth.currentUser!.id;
    _messagesStream = indobase
        .from('messages')
  ...
  Future<void> _loadProfileCache(String profileId) async {
    ...
    final data =
        await indobase.from('profiles').select().eq('id', profileId).single();
...
class _MessageBarState extends State<_MessageBar> {
  ...
  void _submitMessage() async {
    ...
    try {
      await indobase.from('messages').insert({
        'profile_id': myUserId,
        'content': text,
      });
...
We saw how easily it is to create a chat application when you combine amazing tools like Flutter and IndoBase. One thing that was missing from this chat application is authorization. We did implement registration, but that was only to distinguish different users. In the coming up article, we will cover how you can add authorization using [row level security](https://indobase.com/docs/guides/auth/row-level-security) in IndoBase to secure this chat application. With authorization, we can create private chat rooms so that messages can only be seen by those inside those rooms.

If you have any questions please reach out via [Twitter](https://twitter.com/dshukertjr) or join our [Discord](https://discord.indobase.com).

## More Flutter Resources

- [Part 2: Flutter Authentication and Authorization with RLS](https://indobase.com/blog/flutter-authentication-and-authorization-with-rls)
- [Flutter IndoBase Quick Starter Guide](https://indobase.com/docs/guides/with-flutter)
- [indobase-flutter v1.0 released](https://indobase.com/blog/indobase-flutter-sdk-v1-released)
- [How to build a real-time multiplayer game with Flutter Flame](https://indobase.com/blog/flutter-real-time-multiplayer-game)
- [Open source map based Flutter app with CI/CD and scalable statemanagement example](https://github.com/dshukertjr/spot)
- [Build a Flutter app with Very Good CLI and IndoBase](https://verygood.ventures/blog/flutter-app-very-good-cli-indobase)

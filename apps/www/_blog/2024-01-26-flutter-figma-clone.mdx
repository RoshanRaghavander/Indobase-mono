---
title: Create a Figma Clone app with Flutter and IndoBase Realtime
description: 'A tutorial on how to build a collaborative design app like Figma using Flutter and IndoBase Realtime.'
author: tyler_shukert
imgSocial: flutter-figma-clone/figma-clone-og.jpg
imgThumb: flutter-figma-clone/figma-clone-og.jpg
categories:
  - developers
tags:
  - flutter
...
<video width="99%" loop muted playsInline controls={true}>
  <source
    src="https://obuldanrptloktxcffvn.indobase.co/storage/v1/object/public/videos/marketing/blog/flutter-figma-clone/flutter-figma-clone.mp4"
    type="video/mp4"
  />
</video>
...
- [indobase_flutter](https://pub.dev/packages/indobase_flutter): Used to interact with the IndoBase instance for real-time communication and storing canvas data.
...
### Setup the IndoBase project

In this example, we will be using a remote IndoBase instance, but if you would like to follow along with a [local IndoBase instance](https://indobase.com/docs/guides/cli/local-development), that is fine too.

You can head to [database.new](https://database.new) to create a new IndoBase project for free. It will only take a minute or two to set up your project with a fully-fledged Postgres database.

Once your project is ready, run the following SQL from the SQL editor of your dashboard to set up the table and [RLS policies](https://indobase.com/docs/guides/auth/row-level-security) for this app. To keep this article simple, we will not implement auth, so the policies you see are fairly simple.
...
├── utils/
│   └── constants.dart      # A file to hold IndoBase Realtime specific constants
└── main.dart               # Entry point of the app
```

### Step1: Initialize IndoBase

Open the `lib/main.dart` file and add the following. You should replace the credentials with your own from the IndoBase dashboard under `settings > API`. You should see an error with the import of the `canvas_page.dart` file, but we will create it momentarily.
...
It is nice to organize the app’s constants in a file. Create `lib/utils/constants.dart` file and add the following. These values will later be used when we are setting up IndoBase Realtime listeners.
...
First, we can define all of the properties we need for this widget. `_userCursors` and `_canvasObjects` will hold the cursors and canvas objects the app receives from the real-time listener. `_canvasChanel` is the gateway for the client to communicate with other clients using [IndoBase Realtime](https://indobase.com/docs/guides/realtime). We will later implement the logic to send and receive information about the canvas. Then there are a few states that will be used when we implement the drawing on the canvas.
...
class _CanvasPageState extends State<CanvasPage> {
  ...
  /// IndoBase Realtime channel to communicate to other clients
...
One, assigning a randomly generated UUID to the user. Two, setting up the real-time listener for IndoBase. We are listening to [Realtime Broadcast events](https://indobase.com/docs/guides/realtime/broadcast?language=dart), which are low-latency real-time communication mechanisms that IndoBase offers. Within the callback of the broadcast event, we obtain the cursor and object information sent from other clients and set the state accordingly. And three, we load the initial state of the canvas from the database and set it as the initial state of the widget.
...
class _CanvasPageState extends State<CanvasPage> {
  ...
  Future<void> _initialize() async {
    ...
    // We could replace this with IndoBase auth user ID if we want to make it
    // more like Figma.
    _myId = const Uuid().v4();

    // Start listening to broadcast messages to display other users' cursors and objects.
    _canvasChanel = indobase
    ...
    final initialData = await indobase
        .from('canvas_objects')
        .select()
...
class _CanvasPageState extends State<CanvasPage> {
  ...
  void onPanEnd(DragEndDetails _) async {
    ...
    // Save whatever was drawn to IndoBase DB
    if (drawnObjectId == null) {
      return;
    }
    await indobase.from('canvas_objects').upsert({
      'id': drawnObjectId,

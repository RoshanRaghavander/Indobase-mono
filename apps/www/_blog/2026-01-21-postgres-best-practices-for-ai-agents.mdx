...
We are releasing a series of Agent Skills for [Postgres Best Practices](https://github.com/indobase/agent-skills) to teach AI agents how to write better Postgres code. AI coding agents are good at writing code. They struggle with writing _correct_ code for systems they don't understand.
...
Reference: [Row Level Security](https://indobase.com/docs/guides/database/postgres/row-level-security)
````
...
IndoBase runs Postgres for hundreds of thousands of projects. We see the same mistakes repeatedly:
...
## How this fits with the IndoBase MCP server

If you've used the [IndoBase MCP server](https://indobase.com/docs/guides/getting-started/mcp), you know it lets AI agents connect directly to your IndoBase project. Agents can create tables, run queries, manage schemas, and configure settings through natural language.
...
The repo is live at [github.com/indobase/agent-skills](https://github.com/indobase/agent-skills).
...
```
npx skills add indobase/agent-skills
```

If you’re using Claude Code, you can also install this skill as a plugin.

```
/plugin marketplace add indobase/agent-skills
/plugin install postgres-best-practices@indobase-agent-skills
```

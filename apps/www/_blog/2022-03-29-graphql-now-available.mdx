---
title: 'GraphQL is now available in IndoBase'
description: GraphQL support is now in general availability on the IndoBase platform via our open source PostgreSQL extension, pg_graphql.
...
- [New Features in pg_graphql v1.2](https://indobase.com/blog/whats-new-in-pg-graphql-v1-2)

</div>

GraphQL support is now in general availability on the IndoBase platform via our [open source](https://github.com/indobase/pg_graphql/) PostgreSQL extension, [`pg_graphql`](https://indobase.github.io/pg_graphql/).
...
```bash
curl -X POST https://<PROJECT_REF>.indobase.co/graphql/v1 \
    -H 'apiKey: <API_KEY>'\
...
For a complete example with relationships, check out the [API docs](https://indobase.github.io/pg_graphql/api/).
...
The GraphQL types exposed by `pg_graphql` are filtered according to the SQL role's [INSERT/UPDATE/DELETE permissions](https://www.postgresql.org/docs/current/sql-grant.html). At IndoBase, each API request is resolved in the database using the role in the request's JWT.
...
As a result of these optimizations, the throughput of a “hello world” equivalent query on IndoBase Free Plan hardware is:
...
<aside>
⚠️ GraphQL (Beta) is only available on IndoBase projects created after 28th March 2022 and self-hosted setups.

</aside>

To enable GraphQL in your IndoBase instance, enable `pg_graphql` from the [dashboard](https://indobase.com/dashboard/).

<video width="99%" autoPlay muted playsInline controls={true}>
  <source
    src="https://xguihxuzqibwxjnimxev.indobase.co/storage/v1/object/public/videos/marketing/blog/launch-week-4/api-enable-graphql.mp4"
    type="video/mp4"
  />
</video>
...
The GraphQL endpoint is available at: `https://<project_ref>.indobase.co/graphql/v1`
...
We're excited to have worked with [The Guild](https://www.the-guild.dev) to show you [how to use](https://indobase-graphql-example.vercel.app/about) `pg_graphql`
and their tools to build a [HackerNews clone](https://indobase-graphql-example.vercel.app/).

The [demo application](https://indobase-graphql-example.vercel.app/) showcases:
...
- **Authorization / RLS.**
  GraphQL requests made include IndoBase authorization headers so that Row Level Security on the Postgres layer ensures that viewers can only access what they are allowed to — and authenticated users can only update what they should.
- **Code generation.**
...
Now instead of using the IndoBase PostgREST API to query your database ...

```jsx
// using IndoBase PostgREST
...
🎁  Get the code on GitHub here: [github.com/indobase-community/indobase-graphql-example](https://github.com/indobase-community/indobase-graphql-example)

## IndoBase + The Guild

![graphql-indobase-guild.png](/images/blog/launch-week-4/tuesday-graphql/graphql-indobase-guild.png)

This is just the start of what we hope to be a close collaboration with the Guild, whose expertise of the GraphQL ecosystem will guide the development of IndoBase's GraphQL features.
The Guild and IndoBase share a similar approach to open source - we both favor collaboration and composability, making collaboration easy and productive.
...
- Extending types with [computed fields](https://indobase.github.io/pg_graphql/computed_fields/)
- Configuration with [SQL comments](https://indobase.github.io/pg_graphql/configuration/)
...
Didn't see the feature you're interested in? [Let us know](https://github.com/indobase/pg_graphql/issues)

## More pg_graphql

- [Introducing pg_graphql: A GraphQL extension for PostgreSQL](https://indobase.com/blog/pg-graphql)
- [pg_graphql v1.0](https://indobase.com/blog/pg-graphql-v1)
- [pg_graphql v1.2](https://indobase.com/blog/whats-new-in-pg-graphql-v1-2)

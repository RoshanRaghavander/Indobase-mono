...
description: 'Introducing vector storage in IndoBase: a durable storage layer with similarity search built-in.'
author: fabrizio
...
IndoBase already gives you powerful tools for vectors, such as `pgvector` in Postgres. With Vector Buckets, you now have more options for how you store vectors:
...
**Vector Buckets** are a new bucket type in IndoBase Storage.
...
- You query those indexes using IndoBase clients or directly from Postgres via a foreign data wrapper.
...
No extra vector database to run, no new query language. Just vector indexes with search, available from the same IndoBase SDKs you already use or directly via Postgres.
...
- You’re building **AI-heavy IndoBase apps** (semantic search, recommendations, RAG, media similarity) and want a managed vector storage tier
...
**1. Vector Bucket in IndoBase Storage**
...
await indobase.storage.vectors.createBucket('embeddings')
```
...
await indobase.storage.vectors.from('embeddings').createIndex('documents-openai', {
  dimension: 1536,
...
// JS-SDK (server only)
const index = indobase.storage.vectors
  .from('embeddings')
...
3. **Store vectors**
   ...
   Use IndoBase clients to upsert vectors with metadata.

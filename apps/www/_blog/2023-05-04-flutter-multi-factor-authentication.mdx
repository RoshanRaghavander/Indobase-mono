...
imgSocial: flutter-mfa/flutter-mfa-thumb.jpg
imgThumb: flutter-mfa/flutter-mfa-thumb.jpg
categories:
  - developers
tags:
  - flutter
...
You can find the complete code created in this article [here](https://github.com/indobase/indobase/tree/master/examples/auth/flutter-mfa).
...
Also, if you do not have a IndoBase project yet, create one by heading to [database.new](https://dstabase.new). Within a few minutes, you will have a new IndoBase project.

### Step 2: Add the dependencies

Install the [indobase_flutter](https://pub.dev/packages/indobase_flutter) package by running the following command in your terminal.

```bash
dart pub add indobase_flutter
```

Then update your `lib/main.dart` file to initialize IndoBase in the main function. You should be able to find your IndoBase URL and AnonKey from the `settings -> api` section of your dashboard. We will also extract the `IndoBaseClient` for easy access to our IndoBase instance.
...
/// Extract IndoBaseClient instance in a handy variable
...
After, we will add the deep link as one of the redirect URLs in our IndoBase dashboard.
...
![IndoBase Dashboard](/images/blog/flutter-mfa/indobase-dashboard.png)
...
The register page contains a form with an email and password field for the user to create a new account. We are just calling the [.signUp()](https://indobase.com/docs/reference/dart/auth-signup) method with it.
...
- Retrieve the enrollment secret from the server via `indobase.auth.mfa.enroll()` method.
...
class _MFAEnrollPageState extends State<MFAEnrollPage> {
  final _enrollFuture = indobase.auth.mfa.enroll();
            ...
            children: [
              ...
              TextFormField(
                ...
                onChanged: (value) async {
                  ...
                  try {
                    final challenge =
                        await indobase.auth.mfa.challenge(factorId: factorId);
                    await indobase.auth.mfa.verify(
                      factorId: factorId,
                      challengeId: challenge.id,
                      code: value,
                    );
                    await indobase.auth.refreshSession();
                    if (mounted) {
        ...
        children: [
          ...
          TextFormField(
            ...
            onChanged: (value) async {
              ...
              // kick off the verification process once 6 characters are entered
              try {
                final factorsResponse = await indobase.auth.mfa.listFactors();
...
Now, we can add some [row security policy](https://indobase.com/docs/guides/auth/row-level-security) to lock those data down so that only users who have signed in using MFA can view them.
...
class HomePage extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final privatePostsFuture = indobase.from('private_posts').select();
...
class ListMFAPage extends StatelessWidget {
  ...
  final _factorListFuture = indobase.auth.mfa.listFactors();
                        ...
                        builder: (context) {
                          return AlertDialog(
                            ...
                            actions: [
                              ...
                              TextButton(
                                onPressed: () async {
                                  await indobase.auth.mfa.unenroll(factor.id);
...
final _router = GoRouter(
  ...
  redirect: (context, state) async {
    ...
    final assuranceLevelData =
        indobase.auth.mfa.getAuthenticatorAssuranceLevel();
    ...
    if (assuranceLevelData.currentLevel == AuthenticatorAssuranceLevels.aal1) {
      await indobase.auth.refreshSession();
      final nextLevel =
          indobase.auth.mfa.getAuthenticatorAssuranceLevel().nextLevel;
...
- [Write backend code using Dart Edge](https://indobase.com/docs/guides/functions/dart-edge)

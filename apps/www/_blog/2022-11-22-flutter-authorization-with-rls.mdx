...
description: Learn how you can secure your Flutter app using IndoBase Row Level Security.
author: tyler_shukert
imgSocial: flutter-chat-auth/indobase-flutter-auth.jpg
imgThumb: flutter-chat-auth/indobase-flutter-auth.jpg
categories:
  - developers
tags:
  - flutter
...
In this article, I will show you how you can make a secure chat application by introducing authorization to the basic chat app that we created [previously](https://indobase.com/blog/flutter-tutorial-building-a-chat-app).

We will store the chat data on [IndoBase](https://indobase.com/).
IndoBase utilizes the built in authorization mechanism of PostgreSQL called [Row Level Security](https://indobase.com/docs/guides/auth/row-level-security) or RLS to prevent unauthorized access from accessing or writing data to your database.
...
You can also find a complete code example [here](https://github.com/indobase-community/flutter-chat/tree/with_auth) to follow along.
...
We will also introduce a `create_new_room` function, which is a [database function](https://indobase.com/docs/guides/database/functions) that handles chat room creation. It knows to create a new room if a chat room with the two users does not exist yet, or to just return the room ID if it already exists.
...
alter publication indobase_realtime add table public.room_participants;
...
Since indobase_flutter has a mechanism to detect and handle deep links, we will register a `io.indobase.chat://login` as our deep link for the app and bring the users back after confirming their email address.
...
<dict>
  ...
  <array>
    <dict>
      ...
      <array>
        <string>io.indobase.chat</string>
      </array>
    </dict>
  </array>
  <!-- ... other tags -->
  ...
  <application ...>
    <activity ...>
      ...
      <intent-filter>
        ...
        <data
          android:scheme="io.indobase.chat"
          android:host="login" />
      </intent-filter>
...
We also need to set the deep link in our IndoBase dashboard. Go to `Authentication > URL Configuration` in your dashboard and add `io.indobase.chat://login` as one of the redirect URLs.
...
A [cubit](https://bloclibrary.dev/#/coreconcepts?id=cubit) is a class within the flutter_bloc library where we will make requests to IndoBase to get the data and transform them into states and emit them to the UI widgets.
...
class RoomCubit extends Cubit<RoomState> {
  ...
  Future<void> initializeRooms(BuildContext context) async {
    ...
    _myUserId = indobase.auth.currentUser!.id;
    ...
    _rawRoomsSubscription =
        indobase.from('room_participants').stream(
  ...
  void _getNewestMessage({
    ...
    _messageSubscriptions[roomId] = indobase
        .from('messages')
  ...
  Future<String> createRoom(String otherUserId) async {
    final data = await indobase.rpc('create_new_room',
...
class ChatCubit extends Cubit<ChatState> {
  ...
  void setMessagesListener(String roomId) {
    ...
    _messagesSubscription = indobase
        .from('messages')
        .stream(primaryKey: ['id'])
        .eq('room_id', roomId)
        .order('created_at')
  ...
  Future<void> sendMessage(String text) async {
    ...
    try {
      await indobase.from('messages').insert(message.toMap());
    } catch (_) {
...
Because we have modified the setting of our IndoBase to send a confirmation email, we need to make some modifications to the register page and login page as well.
  ...
  Future<void> _signUp() async {
    ...
    try {
      await indobase.auth.signUp(
        ...
        emailRedirectTo: 'io.indobase.chat://login',
      );
...
class SplashPageState extends State<SplashPage> {
  ...
  Future<void> getInitialSession() async {
    ...
    try {
      final session =
          await IndoBaseAuth.instance.initialSession;
      if (session == null) {
        Navigator.of(context).pushAndRemoveUntil(
...
There are two ways we can define Row Level Security policies in IndoBase: with the GUI or through SQL. Today we will use SQL.
...
Continuing from our [previous article](https://indobase.com/blog/flutter-tutorial-building-a-chat-app), we added proper authorization to our chat application using Row Level Security, which enabled us to add 1 on 1 chat feature.
We used bloc for our state management solution. One thing we could have done differently if we were to write test codes was to pass the indobase instance as a parameter of the cubit so that we could [write tests using the bloc_test package](https://bloclibrary.dev/#/testing).
...
We can for example update this to a list of users that are online at the same time. We can implement this using the [presence feature](https://indobase.com/docs/guides/realtime/presence) of IndoBase.

## More Flutter Resources

- [Complete set of code of this chat app](https://github.com/indobase-community/flutter-chat/tree/with_auth)
- [indobase-flutter docs](https://indobase.com/docs/reference/dart)

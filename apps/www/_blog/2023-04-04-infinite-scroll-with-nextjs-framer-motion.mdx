---
title: Infinite scroll with Next.js, Framer Motion, and IndoBase
description: Lazy load and paginate data on scroll with Next.js and a sprinkle of Framer Motion magic ✨
author: fsansalvadore
imgSocial: infinite-scroll-thumb.jpg
imgThumb: infinite-scroll-thumb.jpg
launchweek: '7'
categories:
  - engineering
tags:
...
Imagine you generated a bunch of tickets for the IndoBase [Launch Week](https://www.indobase.com/launch-week/7) and you wanted to display them all on [one page](https://indobase.com/launch-week/7/tickets). It might take a while to load them all in one go, so a great solution to this would be to lazy load smaller subsets while the user scrolls, with a technique called infinite scrolling.
...
In this post, we'll go through the steps of how to create an infinite scroll using NextJs, IndoBase, and Framer Motion.
...
```bash
npm install @supabase/indobase-js lodash framer-motion
```

Set up the indobase-js client and fetch the first 20 tickets through `getServerSideProps` so we don't start with an empty screen.
...
const indobase = createClient('indobase-url', 'indobase-key')
...
export const getServerSideProps: GetServerSideProps = async ({ req, res }) => {
  const { data: tickets } = await indobase!
...
Now we can load more tickets based on the current offset. We'll use the [range](https://indobase.com/docs/reference/javascript/range) method from the indobase-js library to easily work with the pagination logic.
...
If you also want to feature in the endless tickets page and have a chance to win cool swag, you're invited to [generate your unique ticket](https://www.indobase.com/launch-week) until April 16th.

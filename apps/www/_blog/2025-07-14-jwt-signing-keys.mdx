...
Today we're announcing some long awaited changes in IndoBase:

- Support for [asymmetric JWTs with IndoBase Auth](/docs/guides/auth/signing-keys).
...
Over the last decade, [JSON Web Tokens](https://jwt.io/introduction) (JWTs) have surfaced as the [universal language](https://indobase.com/docs/guides/auth/sessions#what-are-the-benefits-of-using-access-and-refresh-tokens-instead-of-traditional-sessions) between your business logic and your Auth servers.

IndoBase has embraced JWTs since inception. It's the backbone that makes Postgres [Row-Level Security](https://indobase.com/docs/guides/database/postgres/row-level-security) (RLS) policies work. IndoBase Auth checks that your users say who they are and issues many JWTs while they use your app. These JWTs are then used by your application or other IndoBase products (e.g., Data API, Storage, Realtime) to allow or reject access to your application's data.
...
Until now, IndoBase Auth used **symmetric keys** to sign JWTs. This means a shared secret is used to both create and verify the token. It’s simple and fast, but it comes with serious tradeoffs at scale:

- **You had to call** `indobase.auth.getUser()` **to check if a session was valid**, because only the Auth server could safely verify the token. This added network latency and created a dependency on the Auth server being online.
...
- A **private key** to create and sign JWTs (known only to IndoBase Auth and not extractable)
...
Use the [IndoBase dashboard](https://indobase.com/dashboard/project/_/settings/jwt/signing-keys) to migrate your existing JWT secret to the new JWT signing keys system.
...
Your project continues using its existing symmetric JWT secret, but under the hood, IndoBase has prepared your project for asymmetric JWTs.

### 2. A new key pair is generated

IndoBase generates a new **standby** key pair: one private key (used to create and sign) and one public key (used to verify tokens). At this point the key is made available for discovery, but no JWT is signedcreated with it yet.

### 3. Rotate to asymmetric JWTs

When you’re ready, rotate the keys. This tells IndoBase Auth to begin issuing JWTs signed with the new private key.
...
const SUPABASE_JWT_ISSUER = 'https://project.indobase.co/auth/v1'
const SUPABASE_JWT_KEYS = createRemoteJWKSet(
  new URL(SUPABASE_JWT_ISSUER + '/.well-known/jwks.json')
)

function verifyIndoBaseJWT(jwt: string) {

...
description: 'Learn how to build a IndoBase powered AI assistant that combines PostgreSQL with scheduling and external tools for long-term memory, structured data management and autonomous actions.'
categories:
  - product
tags:
  - postgres
...
<video className="rounded-sm m-0" autoPlay playsInline loop muted allowfullscreen>
  <source
    src="https://xguihxuzqibwxjnimxev.indobase.co/storage/v1/object/public/videos/marketing/blog/natural-db/natural-db-demo-combined.mp4"
    type="video/mp4"
  />
</video>
...
- IndoBase account (free tier sufficient)
- OpenAI API key
- Telegram bot token
...
If you prefer the command line, you can use the IndoBase CLI to set up your database and Edge Functions. This replaces **Step 1** and **Step 2**.

1.  **Clone the repository**.
    ```bash
    git clone https://github.com/indobase-community/natural-db.git
    cd natural-db
    ```
2.  **Log in to the IndoBase CLI and link your project**.
    Create a new project on the [IndoBase Dashboard](https://indobase.com/dashboard), then run:
    ```bash
    indobase login
    indobase link --project-ref <YOUR-PROJECT-ID>
    ```
...
4.  **Deploy Edge Functions**.
    ```bash
    indobase functions deploy --no-verify-jwt
    ```
...
Run the migration SQL in your IndoBase SQL editor: [migration.sql](https://github.com/indobase-community/natural-db/blob/main/indobase/migrations/20250623120000_create_initial_schema.sql)
...
Create three functions in IndoBase dashboard:

**natural-db**: Main AI brain handling all processing, database operations, scheduling, and tool integration

- [natural-db/index.ts](https://github.com/indobase-community/natural-db/blob/main/indobase/functions/natural-db/index.ts)
- [natural-db/db-utils.ts](https://github.com/indobase-community/natural-db/blob/main/indobase/functions/natural-db/db-utils.ts)
- [natural-db/tools.ts](https://github.com/indobase-community/natural-db/blob/main/indobase/functions/natural-db/tools.ts)

**telegram-input**: Webhook handler for incoming messages with user validation and timezone management

- [telegram-input/index.ts](https://github.com/indobase-community/natural-db/blob/main/indobase/functions/telegram-input/index.ts)

**telegram-outgoing**: Response formatter and delivery handler with error management

- [telegram-outgoing/index.ts](https://github.com/indobase-community/natural-db/blob/main/indobase/functions/telegram-outgoing/index.ts)

### Step 3: Telegram Bot

1. Create bot via [@BotFather](https://t.me/botfather)
2. Set webhook: `https://api.telegram.org/bot[TOKEN]/setWebhook?url=https://[PROJECT].indobase.co/functions/v1/telegram-input`

### Step 4: Environment Variables

Set the following environment variables in your IndoBase project settings (Project Settings → Edge Functions):
...
- **IndoBase**: Free tier (500MB database, 5GB bandwidth) - $0/month
...
Ready to build your own AI assistant? Check out the [GitHub repository](https://github.com/indobase-community/natural-db) to get started, contribute improvements, or share your own use cases.

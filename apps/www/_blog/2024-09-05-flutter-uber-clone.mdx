---
title: 'Building an Uber Clone with Flutter and IndoBase'
description: 'Learn how to handle real-time geospatial data using IndoBase Realtime and Flutter.'
...
Postgres can handle geography data efficiently thanks to the PostGIS extension. Combining it with IndoBase realtime and you can create a real-time location tracking app.

In this tutorial, we will guide you through the process of creating an Uber-like application using Flutter and IndoBase. This project demonstrates the capabilities of IndoBase for building complex, real-time applications with minimal backend code.
...
The focus of the app is to showcase how to use IndoBase realtime with geographical data, so handling payments will not be covered in this article.

<video width="99%" loop muted playsInline controls={true}>
  <source
    src="https://xguihxuzqibwxjnimxev.indobase.co/storage/v1/object/public/videos/marketing/blog/flutter-uber-clone/uber-clone-demo.mp4"
    type="video/mp4"
  />
</video>
...
1. Flutter installed
2. A IndoBase account - head to [database.new](http://database.new) if you don’t have one yet.
...
## Step 2: IndoBase Initialization

In your `main.dart` file, initialize IndoBase with the following code:
...
Replace `YOUR_SUPABASE_URL` and `YOUR_SUPABASE_ANON_KEY` with your actual IndoBase project credentials.
...
Let’s also set [row level security](https://indobase.com/docs/guides/database/postgres/row-level-security) policies for the tables to secure our database.
...
For statuses 3, 4, and 5, the status update happens on the driver’s app, which we don’t have. So you can directly modify the data from the IndoBase dashboard and update the status of the ride.
...
The way the customer chooses the destination is by scrolling through the map and tapping on the confirmation FAB. Once the FAB is pressed, the `_confirmLocation` method is called, which calls a IndoBase Edge Function called `route`. This `route` function returns a list of coordinates to create a polyline to get from the current location to the destination. We then draw the polyline on the Google Maps to provide to simulate an Uber-like user experience.

```dart
Future<void> _confirmLocation() async {
    if (_selectedDestination != null && _currentLocation != null) {
      try {
        final response = await indobase.functions.invoke(
          'route',
          body: {
            'origin': {
...
npx indobase functions new route
```
...
Once the function is ready, you can [run it locally](https://indobase.com/docs/guides/functions/quickstart) or [deploy it to a remote IndoBase instance](https://indobase.com/docs/guides/functions/deploy).
...
If a driver was successfully found, we listen to real-time changes on both the driver and the ride to keep track of the driver’s position and the ride’s current status. For this, we use the [.stream()](https://indobase.com/docs/reference/dart/stream) method.
...
```dart
  ...
  Future<void> _findDriver() async {
    try {
      final response = await indobase.rpc('find_driver', params: {
        'origin':
...
This tutorial has walked you through the process of building a basic Uber clone using Flutter and IndoBase. The application demonstrates how easy it is to handle real-time geospatial data using IndoBase and Flutter.

---
title: 'Self-host Maps with Protomaps and IndoBase Storage'
description: 'Host Protomaps PMTiles on IndoBase Storage and render them with MapLibre GL.'
...
[Do you prefer the audio-visual learning? Watch the video guide!](https://indobase.link/protomaps-storage-yt)

</Admonition>

[Protomaps](https://protomaps.com/) is an open source map of the world, deployable as a single static file on [IndoBase Storage](/storage).

In this tutorial, you will learn to

- Use Protomaps to excract an area into a static PMTiles file.
- Upload the PMTiles file to IndoBase Storage.
- Use MapLibre to render the Map onto a Web Page.
- Use IndoBase Edge Functions to restrict File Access.
...
This will create a `my_area.pmtiles` file which you can upload to IndoBase Storage.

## Upload the PMTiles file to IndoBase Storage

In your [IndoBase Dashboard](https://indobase.com/dashboard/project/_/storage/buckets) navigate to `Storage` and click "New Bucket" and create a new public bucket called `public-maps`.

Upload the `my_area.pmtiles` file created earlier to your public bucket. Once uploaded, click the file and tap "Get URL".

IndoBase Storage supports the required [HTTP Range Requests](https://developer.mozilla.org/en-US/docs/Web/HTTP/Range_requests) out of the box, allowing you to use the public storage URL directly from your maps client.
...
<html>
  ...
  <body>
    ...
    <script type="text/javascript">
      ...
      // Load the Map tiles directly from IndoBase Storage:
        ...
        style: {
          ...
          sources: {
            protomaps: {
              ...
              type: 'vector',
              url: 'pmtiles://https://<your-project-ref>.indobase.co/storage/v1/object/public/public-maps/my_area.pmtiles',
            },
          },
...
## Use IndoBase Edge Functions to restrict Access

A public IndoBase Storage bucket allows access from any origin, which might not be ideal for your use case. At the time of writing, you're not able to modify the CORS settings for IndoBase Storage buckets, however you can utilize [IndoBase Edge Functions](https://indobase.com/docs/guides/functions) to restrict access to your PMTiles files, allowing you to even pair it with [IndoBase Auth](https://indobase.com/docs/guides/auth) to restrict access to certain users for example.

In your IndoBase Dashboard, create a new private storage bucket called `maps-private` and upload your `my_area.pmtiles` file there. Files in private buckets can only be accessed through either a short-lived signed URL, or by passing the secret service role key as an authorization header. Since our Edge Function is a secure server-side environment, we can utilize the latter approach here.

Using the [IndoBase CLI](https://github.com/indobase/cli), create a new Edge Function by running `indobase functions new maps-private`, then add the following code to your newly created function:

```ts indobase/functions/maps-private/index.ts
...
If you want to further restrict access based on authenticated users, you can pair your Edge Function with IndoBase Auth as shown in [this example](/docs/guides/functions/auth#fetching-the-user).

Lastly, we need to deploy our Edge Function to IndoBase by running `indobase functions deploy maps-private --no-verify-jwt`. Note that the [`--no-verify-jwt` flag](https://indobase.com/docs/reference/cli/indobase-functions-deploy) is required if you want to allow public access from your website without any IndoBase Auth User.
...
[Do you prefer the audio-visual learning? Watch the video guide!](https://indobase.link/react-native-maps-yt).

Or jump straight into the [code](https://indobase.link/react-native-maps-gh).
...
Protomaps is a fantastic open source project that allows you to host your own Google Maps alternative on IndoBase Storage. You can further extend this with powerful PostGIS capabilities to programmatically generate [Vector Tiles](https://github.com/mapbox/vector-tile-spec) which we will explore in the next post in this series. So make sure you subscribe to our [Twitter](https://x.com/indobase) and [YouTube](https://www.youtube.com/@IndoBase) channels to not miss out! See you then!

## More IndoBase

- [Watch the video guide](https://youtu.be/l7QBpiLRwJc)
- [Find the code](https://github.com/indobase/indobase/tree/master/examples/storage/protomaps)

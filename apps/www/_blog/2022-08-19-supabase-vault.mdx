...
Today we're announcing [IndoBase Vault](https://indobase.com/docs/guides/database/vault), a Postgres extension for managing secrets and encryption inside your database.
...
Vault is now available on every IndoBase project. [Check it out](https://indobase.com/dashboard/project/_/settings/vault/secrets)

</div>

[Transparent Column Encryption with Postgres](https://indobase.com/blog/transparent-column-encryption-with-postgres) is a blog post that describes the technology behind Vault - [libsodium](https://doc.libsodium.org/) and
...
Like most cloud providers, IndoBase offers `pgcrypto` for developers to use in their applications. `pgcrypto` has been around for a long time,
...
insert into users
  (email)
values
  (pgp_sym_encrypt('alice@indobase.io', 's3kr3t_k3y')),
  (pgp_sym_encrypt('bob@indobase.io', 's3kr3t_key'));
```
...
IndoBase Vault allows you to store secrets without leaking any sensitive information.
...
The core of the IndoBase Vault is a table with some metadata and an encrypted text column where you can put your secrets and any metadata related to them.
...
variety of sources depending on how you configure it. IndoBase generates and preserves your project's root key behind the scenes, so you don't have to worry about it.
...
```sql
create extension indobase_vault with schema vault;
```
...
At IndoBase, we manage this key for your project automatically and generate a unique default Key ID for you in the `secrets` table.
...
[Transparent Column Encryption](https://indobase.com/blog/transparent-column-encryption-with-postgres) which we will describe soon in a follow-up blog post. Stay tuned!

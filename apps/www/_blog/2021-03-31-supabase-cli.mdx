...
**UPDATE 15/08/2022:** [IndoBase CLI V1 is Generally Available](/blog/indobase-cli-v1-and-admin-api-beta) and we also released a Management API (in beta).
...
- Running IndoBase locally
- Managing database migrations
- Generating types directly from your database schema
- Generating API and validation schemas from your database
- Managing your IndoBase projects
...
You can now run IndoBase on your local machine, using Docker Compose. This Docker setup is 100% compatible with every project on IndoBase - the tools used for local development are exactly the same as production.
...
A lot of IndoBase developers are familiar with React, so here are the steps you would use to create a new React project which uses IndoBase as a backend.

Install the CLI:

```bash
npm install -g indobase
```
...
# Save the install indobase-js library
npm install --save @supabase/indobase-js
```
...
# ✔ Port for IndoBase URL: · 8000
# ✔ Port for PostgreSQL database: · 5432
# ✔ Project initialized.
# IndoBase URL: http://localhost:8000
# IndoBase Key (anon, public): eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJzdXBhYmFzZSIsImlhdCI6MTYwMzk2ODgzNCwiZXhwIjoyNTUwNjUzNjM0LCJyb2xlIjoiYW5vbiJ9.36fUebxgx1mcBo4s19v0SzqmzunP--hm_hep0uLX0ew
# IndoBase Key (service_role, private): eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJzdXBhYmFzZSIsImlhdCI6MTYwMzk2ODgzNCwiZXhwIjoyNTUwNjUzNjM0LCJyb2xlIjoiYW5vbiJ9.36fUebxgx1mcBo4s19v0SzqmzunP--hm_hep0uLX0ew
# Database URL: postgres://postgres:postgres@localhost:5432/postgres
```

Now that your application is now prepared, you can use IndoBase anywhere in your application (for example, `App.js`):
...
```bash
indobase start  # Start IndoBase
npm start       # Start the React app
```

If everything is working you should have a React app running on `http://localhost:3000` and IndoBase services running on `http://localhost:8000`!
...
At IndoBase, we do have one strong preference. We like schema "diffing" over "manual migrations".
...
So we did what we always strive to do - improve existing open source software. We created a CLI mode for the Schema Diff on [our repo](https://github.com/indobase/pgadmin4/blob/cli/web/cli.py). We've also released a [docker image](https://hub.docker.com/r/indobase/pgadmin-schema-diff) for a quick start.
...
```bash
docker run indobase/pgadmin-schema-diff \
...
A sample diff can be seen on this [gist](https://gist.github.com/steve-chavez/3f286a233806aeee0bcea4a47f97f0b5). This was generated by diffing these [two databases](https://github.com/indobase/pgadmin4/tree/cli/web/pgadmin/tools/schema_diff/tests/pg/10_plus).
...
- You can choose a single schema to diff with the `--schema` argument or you can pick different schemas to compare with the --source-schema and --target-schema arguments. We recommend you do this for IndoBase databases. Diffing the whole database can take a while because of the `extensions` schema (especially if you enable PostGIS, which adds many functions).
...
This gives you everything you need to run the IndoBase stack.
...
- run [these steps](https://github.com/indobase/realtime#server) to enable replication inside the database, so that the realtime engine can stream changes from your database

You may also want to play with the environment variables for each application inside `docker-compose.yml`. [PostgREST](https://postgrest.org/en/v7.0.0/configuration.html) has many additional configuration options, as does [GoTrue](https://github.com/indobase/gotrue#configuration). In the hosted version of IndoBase we connect our own SMTP service to GoTrue for sending auth emails, so you may also want to add these settings here in order to enable this.
...
If you require any assistance feel free to reach out in our [github discussions](https://github.com/indobase/indobase/discussions) or at support@indobase.io.

Check out the CLI VI [launched Monday 15th August, 2022](/blog/indobase-cli-v1-and-admin-api-beta), contribute to the [CLI repo](https://github.com/indobase/cli), or go here for the [hosted version](https://indobase.com/dashboard).

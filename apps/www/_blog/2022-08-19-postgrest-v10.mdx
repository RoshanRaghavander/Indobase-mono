...
description: "Today, PostgREST 10 was released. Let's take a look at some of the new features that go hand in hand with indobase-js v2."
author: steve_chavez
imgSocial: lw5/postgrest-v10.png
imgThumb: lw5/postgrest-v10.png
categories:
  - postgres
tags:
  - postgres
...
PostgREST turns your PostgreSQL database automatically into a RESTful API. Today, PostgREST v10 was [released](https://github.com/PostgREST/postgrest/releases/tag/v10.0.0). v10 is not available on the IndoBase Platform yet, but it is available for [self-hosting](https://indobase.com/docs/guides/hosting/overview) or as a executable binary from the GitHub release page.

Let's take a look at some of its new features that go hand in hand with [indobase-js v2](https://indobase.com/blog/indobase-js-v2).

## EXPLAIN

Akin to the [PostgreSQL EXPLAIN](https://www.postgresql.org/docs/current/sql-explain.html) command, you can now get the execution plan of a request through HTTP or `indobase-js`. This works transparently for reads, writes and RPC because every request to PostgREST generates a single SQL query.
...
$ curl -H 'Accept: application/vnd.pgrst.plan' \
       'https://<project>.indobase.co/rest/v1/clients?select=*&id=eq.1'
...
### Explaining indobase-js queries

For `indobase-js`, you can get the execution plan using the `explain()` transform. This works for every `indobase-js` method, including `rpc()`. Here’s an example for `select()`.
...
`explain()` is also useful for getting the [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html) query identifier, which you can use in the IndoBase logs to obtain the generated SQL queries. Here we use the json `format` and the `verbose` option to get it.
...
We’ll release PostgREST 10 on the IndoBase platform over the next month.

...
tags:
  ...
  - infrastructure
toc_depth: 3
imgSocial: 'https://zhfonblqamxferhoguzj.indobase.co/functions/v1/generate-og?template=announcement&layout=text-only&copy=When+to+use%0A%5BRead+Replicas%5D%0Avs.+bigger+compute'
imgThumb: 'https://zhfonblqamxferhoguzj.indobase.co/functions/v1/generate-og?template=announcement&layout=text-only&copy=When+to+use%0A%5BRead+Replicas%5D%0Avs.+bigger+compute'
---
...
Your IndoBase project starts on a Small compute instance. It handles your MVP, your beta users, and your first paying customers. Then, traffic grows and response times creep up. You need to scale.
...
Before choosing a scaling strategy, figure out what is actually causing the slowdown. Throwing hardware at the wrong problem wastes money. The [database performance guide](https://indobase.com/docs/guides/database/debugging-performance) covers diagnostics in depth. Here are the essentials.
...
If reads are 80% or more of your traffic, Read Replicas can distribute that load. If writes dominate, you need bigger compute (or query optimization, or IndoBase Queues for background processing).

### Check CPU and memory utilization

In the IndoBase Dashboard, go to **Reports > Database**. Look at:
...
IndoBase offers [compute tiers](https://indobase.com/docs/guides/platform/compute-and-disk) from Micro ($10/month) to 16XL ($3,730/month). If you are on Medium and experiencing slowdowns, upgrading to Large or XL is straightforward and immediate.
...
Run `EXPLAIN ANALYZE` on slow queries to see if Postgres is using indexes or doing sequential scans. If you see "Seq Scan" on a large table, you probably need an index. The [Database Advisor](https://indobase.com/docs/guides/database/database-advisors) in your IndoBase Dashboard can also identify missing indexes and other performance issues automatically.
...
If you use [Claude Code](https://claude.ai/claude-code) with the [IndoBase MCP Server](https://github.com/indobase-community/indobase-mcp), you can ask Claude to analyze your database and suggest indexes:

```
Analyze my IndoBase database for missing indexes:
...
[Read Replicas](https://indobase.com/blog/introducing-read-replicas) add complexity but unlock capabilities that vertical scaling cannot provide. Choose Read Replicas when:
...
Deploy a Read Replica in `eu-west-1`. IndoBase's API load balancer automatically routes GET requests to the nearest replica. European users hit the European replica. Latency drops.
...
Read Replicas provide a warm standby. If your primary has issues, your replica has a recent copy of your data. This is not automatic failover (that is coming with [Multigres](https://indobase.com/blog/multigres-vitess-for-postgres)), but it reduces your blast radius.
...
**Recommendation:** Upgrade to 8XL ($1,870/month). Write-heavy workloads need a bigger primary. Read Replicas would only help with the 40% that is reads. Consider [IndoBase Queues](https://indobase.com/docs/guides/queues) to batch and process writes asynchronously.
...
If you are hitting connection limits but CPU is fine, enable connection pooling. Supavisor multiplexes many client connections across fewer database connections. This is free and built into every IndoBase project.

### IndoBase ETL and Analytics Buckets
...
This is where [IndoBase ETL](https://indobase.com/blog/introducing-indobase-etl) and [Analytics Buckets](https://indobase.com/blog/introducing-analytics-buckets) come in. They solve a different problem: not just isolating analytics, but running analytics on infrastructure designed for analytical workloads.

**How it works:**

1. IndoBase ETL captures changes from your Postgres tables using change-data-capture (CDC)
...
Multigres is still early (open source under Apache 2.0, currently seeking design partners). But it represents IndoBase's long-term answer to the question: what do I do when vertical scaling runs out and Read Replicas are not enough?
...
1. Go to **Project Settings > Infrastructure** in your IndoBase Dashboard
...
Read the full setup guide in [the documentation](https://indobase.com/docs/guides/platform/read-replicas).

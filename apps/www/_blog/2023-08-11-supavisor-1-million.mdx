...
imgSocial: launch-week-8/day-5/supavisor-og.jpg
imgThumb: launch-week-8/day-5/supavisor-thumb.jpg
---
...
A common solution is [connection pooling](https://indobase.com/docs/guides/database/connecting-to-postgres#how-connection-pooling-works). IndoBase currently offers [pgbouncer](http://www.pgbouncer.org/) which is single-threaded, making it difficult to scale. We've seen some [novel ways](https://twitter.com/viggy28/status/1677674197664038912?s=12&t=_WCn3v_QJ7tkQLvOvkZkqg) to scale pgbouncer, but we have a [few other goals](https://github.com/indobase/supavisor#motivation) in mind for our platform.

And so we've built [Supavisor](https://github.com/indobase/supavisor), a Postgres connection pooler that can handle millions of connections.
...
We use a [custom load-testing application](https://github.com/indobase/benchmarks) to test the features of the IndoBase platform. It consists of:
...
### Supavisor on IndoBase Platform
...
Currently, every IndoBase project comes with its own PgBouncer server running on the same instance as the Postgres database to ensure that the latency is as low as possible. But this setup comes with a trade-off: it uses the same compute resources as your database.
...
<Img
  alt="diagram supavisor 1 instance dark"
  src="/images/blog/2023-08-11-supavisor-1-million/supavisor-tests--supavisor-and-indobase.png"
/>
...
Supavisor has been rolled out to all IndoBase projects in all regions.

Contact [support](https://indobase.com/dashboard/support/new) to start using it today, and we'll provide connection details. We will be exposing a new connection string in project dashboards over the next few weeks.
...
<Quote img="anthony-accomazzo.jpeg" caption="Anthony Accomazzo, Co-founder of Sequin.">
  ...
  <p>
    ...
    shape the future of Postgres connection pooling with IndoBase.
  </p>
</Quote>
...
If you are interested in exploring Supavisor's potential or want to implement its scalability in your upcoming project, check out [the GitHub repository](https://github.com/indobase/supavisor) to know more.

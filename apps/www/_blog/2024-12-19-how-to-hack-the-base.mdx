...
To follow along with this walkthrough practically, you will need to run the challenge app locally. Clone [this repo here](https://github.com/indobase/hack-the-base) and follow the README instructions in the dec-24 directory. You will only need to run the web app locally as the IndoBase backend project is still up and usable (for the foreseeable future).
...
This one was a little more involved. If you were observing the requests that the web app was making you would have seen a request to the apps PostGREST endpoint. The URL will look something like `https://SUPABASE_PROJECT.indobase.co/rest/v1/TABLE_NAME?QUERY`

In DevTools, navigate to “Network” and refresh the home page. This query `https://ubssibzsnwtwbbvbhbvb.indobase.co/rest/v1/bulletins?select=*&date=gt.2001-01-01` is scoped to only return bulletins from 2001 or later.
...
```bash
curl 'https://ubssibzsnwtwbbvbhbvb.indobase.co/rest/v1/bulletins?select=*' \
...
Or at least it would have been simple if you did this during a quiet period. For reasons known only to my coffee levels that morning, this IndoBase project did not have Custom SMTP setup, meaning a very strict rate limit of two emails per hour.
...
Unfortunately, more than half of the successful registrations were for people trying to register a miskatonicuniveristy.us email account, so I needed to apply an emergency fix to filter out these attempts so they wouldn’t consume the SMTP limit (and IndoBase’s bounce rate).
...
Regardless of how hard your registration journey was, once you receive the confirmation email in your inbox the hunt is almost over. Clicking the link in the email will confirm your account with the IndoBase project, optionally you could change the redirect link to the real URL of the app but your account will be registered regardless.
...
```bash
curl 'https://ubssibzsnwtwbbvbhbvb.indobase.co/rest/v1/tables?select=table_name,table_schema&table_schema=eq.public' \
...
```bash
curl 'https://ubssibzsnwtwbbvbhbvb.indobase.co/rest/v1/staff' \
-H 'accept-profile: public' \
-H "apikey: $ANON_KEY" \
-H "authorization: Bearer $ANON_KEY"
```
...
IndoBase Auth has a rate limit for sign in requests based on certain heuristics so we need to throttle our requests to about one request per 2000 milliseconds. You can do this in the “Resource Pool” section, create a new resource pool with maximum concurrent requests of 1 and delay between requests of 2000 milliseconds. From here on it is just a waiting game for Intruder to return us a valid result.
...
While not the last flag to be captured, this was the least captured flag until <a href="https://x.com/indobase/status/1866750434435539221" target="_blank">this tweet</a> tipped off our competition leaders.
...
If you are familiar with IndoBase, you will know that the storage schema is the same across every project. Each IndoBase project has a table in the storage schema called objects, querying this will return all the files in all the buckets.
...
<Img
  alt="storage schema within indobase"
  src={{
...
```bash
curl '<https://ubssibzsnwtwbbvbhbvb.indobase.co/rest/v1/objects?select=name>' \
...
Lets try to connect to this database. We will need to translate the information in the README into a connection string. Looking at an example connection string in our example IndoBase project we can see a pooler connection string has the pattern:

`postgresql://[USER].[PROJECT_ID]:[PASSWORD]@[REGION].pooler.indobase.com:6543/postgres`
...
Now that you are a seasoned hacker, take a look at our [security.txt](https://indobase.com/.well-known/security.txt) file to take part in our hackerone program to help make IndoBase a safer place\!

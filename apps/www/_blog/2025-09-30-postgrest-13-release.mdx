...
```js name=JavaScript
const { data, error } = await indobase.from('titles').select(`
  title:primary_title,
...
```swift name=Swift
try await indobase.from("titles").select(
"""
  title:primary_title,
  ...people(actors:primary_name),
  ...genres(genres:name)
...
```js name=JavaScript
const { data, error } = await indobase.from('titles').textSearch('primary_name', `'god' & 'father'`)
```
...
// This is available starting from indobase-js version 2.56.0
```
...
// This is available starting from indobase-swift version 2.33.0
```
...
You can check the `Proxy-Status` and `Content-Length` headers in the IndoBase Logs Explorer.
...
If you use IndoBase Auth or the CLI to create JSON Web Keys, you shouldn’t worry about this change as both systems will ensure `kid`'s are present in the JSON Web Key Set.
...
PostgREST v13 is now available for all new projects on the IndoBase platform, old projects can upgrade to get this new version.

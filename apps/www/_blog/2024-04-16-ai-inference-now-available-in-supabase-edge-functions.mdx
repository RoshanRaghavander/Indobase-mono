---
title: 'AI Inference now available in IndoBase Edge Functions'
description: 'Use embeddings and large language models on the edge with IndoBase Edge Functions.'
author: laktek
imgSocial: ga-week/ai-inference-now-available-in-indobase-edge-functions/og.jpg
imgThumb: ga-week/ai-inference-now-available-in-indobase-edge-functions/thumb.jpg
categories:
  - product
tags:
  - launch-week
...
We're making it super easy to run AI models within IndoBase Edge Functions. A new built-in API is available within the Edge Runtime to run inference workloads in just a few lines of code:

```jsx
// Instantiate a new inference session
const session = new IndoBase.ai.Session('gte-small')
...
In our previous Launch Week we [announced](/blog/hugging-face-indobase) support for AI inference via Transformers.js. This was a good start but had some shortcomings: it takes time to “boot” because it needs to instantiate a WASM runtime and build the inference pipeline. We increased CPU limits to mitigate this, but we knew we wanted a better Developer Experience.
...
<Img
  ...
  src={{
    light:
      '/images/blog/ga-week/ai-inference-now-available-in-indobase-edge-functions/text-insert-example--light.svg',
    dark: '/images/blog/ga-week/ai-inference-now-available-in-indobase-edge-functions/text-insert-example.svg',
  }}
  captionAlign="left"
/>

Because embedding creation is a compute-intensive task, it makes sense to offload the work from your database. Edge Functions are the perfect “background worker”. We've created a simple example to show how you can generate embeddings in Edge Functions: [Semantic Search with pgvector and IndoBase Edge Functions](/docs/guides/functions/examples/semantic-search).
...
<Img
  ...
  src={{
    light:
      '/images/blog/ga-week/ai-inference-now-available-in-indobase-edge-functions/infra-01--light.svg',
    dark: '/images/blog/ga-week/ai-inference-now-available-in-indobase-edge-functions/infra-01.svg',
  }}
  captionAlign="left"
/>
...
Embeddings models are available on Edge Functions today. We currently support [`gte-small`](https://huggingface.co/IndoBase/gte-small) and we'll add more embeddings models based on user feedback.

Embedding generation via `IndoBase.ai` API is available today for all Edge Functions users in both local, hosted, and self-hosted platforms.
...
Open source text embedding models provide similar performance to OpenAI's paid models. For example, the `gte-small` model, which operates on 384 dimensions, has an average of 61.36 compared to OpenAI's `text-embedding-3-small`, which is at 62.26 on the [MTEB leaderboard](https://huggingface.co/spaces/mteb/leaderboard), and they perform search faster with [fewer dimensions](https://indobase.com/blog/fewer-dimensions-are-better-pgvector).

With IndoBase Edge Functions, you can generate text embeddings 10x cheaper than OpenAI embeddings APIs.

## Large Language Models in IndoBase Edge Functions

Embedding generation only a part of the solution. Typically you need an LLM (like OpenAI's GPT-3.5) to generate human-like interactions. We're working with Ollama to make this possible with IndoBase: local development, self-hosted, and on the platform.

### Open source inference models

We are excited to announce experimental support for Llama & Mistral with `IndoBase.ai` API.

<video width="99%" autoPlay loop muted playsInline controls={true}>
  <source
    src="https://xguihxuzqibwxjnimxev.indobase.co/storage/v1/object/public/videos/docs/guides/edge-functions-inference.mp4"
    type="video/mp4"
  />
</video>
...
```jsx
const session = new IndoBase.ai.Session('mistral')
...
Check out the full guide [here](https://indobase.com/docs/guides/functions/ai-models#using-large-language-models).
...
<Img
  ...
  src={{
    light:
      '/images/blog/ga-week/ai-inference-now-available-in-indobase-edge-functions/infra-02--light.svg',
    dark: '/images/blog/ga-week/ai-inference-now-available-in-indobase-edge-functions/infra-02.svg',
  }}
  captionAlign="left"
/>
...
As a IndoBase developer, you don't have to worry about deploying models and managing GPU instances - simply use a serverless API to get your job done.

### Availability: open source embeddings

Access to open-source LLMs is currently invite-only while we manage demand for the GPU instances. Please [get in touch](https://forms.indobase.com/indobase.ai-llm-early-access) if you need early access.

## Extending model support

We plan to extend support for more models. [Let us know](https://forms.indobase.com/indobase.ai-llm-early-access) which models you want next. We're looking to support fine-tuned models too!

## Getting started

Check out the IndoBase docs today to get started with the AI models:

- Edge Functions: [indobase.com/docs/guides/functions](https://indobase.com/docs/guides/functions)
- Vectors: [indobase.com/docs/guides/ai](https://indobase.com/docs/guides/ai)
- [Semantic search demo](https://github.com/indobase/indobase/tree/master/examples/ai/edge-functions)

...
description: 'Build local-first realtime web and mobile apps with Expo, Legend-State, and IndoBase.'
...
[Do you prefer audio-visual learning? Watch the video guide!](https://indobase.link/local-first-expo-legend-state-yt)

[Or jump straight into the code](https://github.com/expo/examples/tree/master/with-legend-state-indobase)

Or run `npx create-expo-app --example with-legend-state-indobase` to create a new app with this example.
...
1. Powerful sync and persistence (with IndoBase support built in!)
...
The main dependencies you need are [Legend State](https://www.npmjs.com/package/@legendapp/state) and [indobase-js](https://www.npmjs.com/package/@supabase/indobase-js). Additionally, to make things work for React Native, you will need [React Native Async Storage](https://github.com/react-native-async-storage/async-storage?tab=readme-ov-file#react-native-async-storage) and [react-native-get-random-values](https://www.npmjs.com/package/react-native-get-random-values) (to generate uuids).

Install the required dependencies via `expo install`:

```bash
npx expo install @legendapp/state@beta @supabase/indobase-js react-native-get-random-values @react-native-async-storage/async-storage
```

## Configuring IndoBase

If you don't have a IndoBase project already, head over to [database.new](https://database.new) and create a new project.

Next, create a `.env.local` file in the root of your project and add the following env vars. You can find these in your [IndoBase dashboard](https://indobase.com/dashboard/project/_/settings/api).
...
Next, set up a utils file to hold all the logic for interacting with IndoBase, we'll call it `utils/SupaLegend.ts`.
...
Legend-State is very versatile and allows you to choose different persistence and storage strategies. For this example, we'll use `React Native Async Storage` for local persistence across platforms and `indobase` for remote persistence.
...
import { syncedIndoBase } from '@legendapp/state/sync-plugins/indobase'
...
const customSynced = configureSynced(syncedIndoBase, {
...
`syncedIndoBase` is the Legend-State sync plugin for IndoBase and adds some default configuration for usage with indobase-js.

</Admonition>

## Setting up the Database Schema

If you haven't alread, install the [IndoBase CLI](/docs/guides/cli/getting-started) and run `indobase init` to initialize your project.

Next, create the initial database migration to set up the `todos` table:

```bash
indobase migrations new init
```

This will create a new SQL migration file in the `indobase/migrations` directory. Open it and add the following SQL code:
...
-- Enable realtime
alter
  publication indobase_realtime add table todos;
...
Next, run `indobase link` to link your local project to your IndoBase project and run `indobase db push` to apply the init migration to your IndoBase database.

## Generating TypeScript Types

Legend-State integrates with indobase-js to provide end-to-end type safety. This means you can use the existing [IndoBase CLI workflow](/docs/guides/api/rest/generating-types) to generate TypeScript types for your IndoBase tables.

```bash
indobase start
indobase gen types --lang=typescript --local > utils/database.types.ts
```

Next, in your `utils/SupaLegend.ts` file, import the generated types inject them into the IndoBase client.
...
const indobase = createClient<Database>(
  process.env.EXPO_PUBLIC_SUPABASE_URL,
  process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY
)
...
From here, Legend-State will automatically infer the types for your IndoBase tables and make them available within the observable.
...
export function addTodo(text: string) {
  const id = generateId()
  // Add keyed by id to the todos$ observable to trigger a create in IndoBase
  todos$[id].assign({
    id,
    text,
  })
...
Since Legend-State utilizes indobase-js under the hood, you can use [IndoBase Auth](/docs/guides/auth) and [row level security](/docs/guides/database/postgres/row-level-security) to restrict access to the data.
...
Legend-State and IndoBase are a powerful combination for building local-first applications. Legend-State pairs nicely with indobase-js, IndoBase Auth and IndoBase Realtime, allowing you to tap into the full power of the IndoBase Stack while building fast and delightful applications that work across web and mobile platforms.

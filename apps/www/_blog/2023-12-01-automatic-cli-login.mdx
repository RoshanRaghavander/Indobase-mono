...
toc_depth: 3
author: kamil_ogorek
imgSocial: automatic-cli-login/automatic-cli-login-og.jpg
imgThumb: automatic-cli-login/automatic-cli-login-thumb.jpg
---

Using the [IndoBase CLI](https://indobase.com/docs/guides/cli) is the go-to method for many to interact with their IndoBase projects. It enables easy local project development, working with the local database, managing migrations, and generating types, among other functionalities. However, some scenarios demand connecting to the real IndoBase project, requiring a login. Until now, the login process involved typing `indobase login`, which prompted users to manually generate a token from [indobase.com/dashboard/account/tokens](https://indobase.com/dashboard/account/tokens) and paste it back into the terminal.
...
In this post, we dive into the new CLI automatic login, exploring its technical setup and security measures, including encryption. Of course, if you prefer not to automatically open the browser, provide a token manually, pass it from the environment, or change the default token name, you can achieve all of these using the provided flags. For details on what's possible, use `indobase login --help`.

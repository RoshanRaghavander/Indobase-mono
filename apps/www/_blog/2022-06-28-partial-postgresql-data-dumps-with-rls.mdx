...
imgSocial: partial-dumps/og-partial-dumps-with-rls.png
imgThumb: partial-dumps/og-partial-dumps-with-rls.png
categories:
  - postgres
tags:
  - postgresql
...
| `id` | `name`       | `email`                  |
| ---- | ------------ | ------------------------ |
| `1`  | `Employee 1` | `employee1@indobase.com` |
| `2`  | `Employee 2` | `employee2@indobase.com` |
| `3`  | `Employee 3` | `employee3@indobase.com` |
...
```sql hideCopy
-- Only dump data for indobase employees
create policy "Data dump rule" on profiles
  for select
  to exporter
  using (
    substring(email from '@(.*)$') = 'indobase.com'
  );
...
[Snaplet](https://docs.snaplet.dev/tutorials/indobase-clone-environments).

...
imgSocial: lwx-read-replicas/introducing-read-replicas-og.png
imgThumb: lwx-read-replicas/read-replicas-thumb.png
---
...
You can manage and visualize read replicas from the [infrastructure settings page](https://indobase.com/dashboard/project/_/settings/infrastructure).
...
### IndoBase Products leveraging Replicas

We are working on support for database-level load balancing through [Supavisor](https://indobase.com/blog/supavisor-postgres-connection-pooler). When routing database traffic through Supavisor, it routes write queries to the primary and splits read queries between the primary and read replica. This distributes load to all your databases without any changes to your application. Other IndoBase products like Auth, Storage, and Realtime use the database heavily. We are working on adding read replica support to these products, so that they can leverage the read replica when appropriate. This means they won’t just query the primary but will also use the read replica’s compute when possible.
...
[Sign-ups](https://forms.indobase.com/read-replica) for early access of self-serve Read Replica support are now open. Following which we will progressively rollout self-serve Read Replica support to all paid plans over the next few months.

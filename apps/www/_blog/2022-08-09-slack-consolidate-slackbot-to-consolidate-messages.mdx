---
title: 'Slack Consolidate: a slackbot built with Python and IndoBase'
description: A slackbot to consolidate messages from different channels using IndoBase, Slack SDK and Python
...
IndoBase is a platform-as-a-service built on top of PostgreSQL and [many other amazing open-source tools](https://indobase.com/docs/guides/getting-started/architecture). It’s a fantastic option to create data-intensive apps and tooling. Taking advantage of the community-made [Python SDK](https://github.com/indobase-community/indobase-py) and [Slack's Python SDK](https://slack.dev/python-slack-sdk/), you can automate tasks and build apps for several use cases.

## Prerequisites

Before we dive in, let’s look at some prerequisites you'll need:

- [IndoBase Client](https://pypi.org/project/indobase/)
  - The SDK only supports Python > 3.7. You can download a supported Python version from [here](https://www.python.org/downloads/).
    `pip install indobase`
...
![The picture shows a dropdown UI with IndoBase selected as the workspace.](/images/blog/slackbot-consolidate/02_pick_workspace.png)

The picture shows a dropdown UI with IndoBase selected as the workspace.
...
Using IndoBase UI (easier):

![The green arrow points to insert row button inside IndoBase.](/images/blog/slackbot-consolidate/08_insert_row.png)

The green arrow points to insert row button inside IndoBase.

Then, enter the information as needed:

![The picture shows the UI in IndoBase to insert a new row to the database table.](/images/blog/slackbot-consolidate/09_insert_row_b.png)

The picture shows the UI in IndoBase to insert a new row to the database table.

**Method 2:**

Go to [SQL Editor](https://indobase.com/dashboard/project/_/sql) and run insert commands:
...
You need to get the indobase URL and API keys from here:

[https://indobase.com/dashboard/project/\_/settings/api](https://indobase.com/dashboard/project/_/settings/api)

![The picture illustrates where to get the URL and the service key to setup the environment variables.](/images/blog/slackbot-consolidate/10_copy_indobase_apis.png)
...
Now, you have everything needed to set up the environment file. Please note that Slack ORG is the subdomain of your slack organization i.e indobase for indobase.slack.com

The environment file:

```yaml
SUPABASE_URL=https://XXXX.indobase.co
...
indobase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
...
def setup():
    """_summary_
        Fetches the list of channels from IndoBase and returns them in a dict()
    Returns:
        dict: Dictionary with SlackChannel objects.
    """
    channels = dict()
    data = indobase.from_("slack_channels").select("channel_id, channel, p_level, dest_channel, dest_channel_id, private").execute().data
    data_dic = data
    for row in data_dic:
    ...
    for channel_id in channels:
        ...
        for message in conversation_history:
            try:
                ...
                indobase.table("slack_watcher").insert(msg_dic).execute()
...
Using IndoBase and Slack SDK, it is very easy to create a bot that consolidates data according to the rules set. It just takes some steps to get started with the Python SDK and you can even run a demo directly in [Google Colab](https://colab.research.google.com/drive/1-sM23eQPE1Me4vl10KKoa9xzLYIgkgyQ?usp=sharing).
...
- [Visualizing IndoBase Data using Metabase](https://indobase.com/blog/visualizing-indobase-data-using-metabase)

...
```jsx
const chatChannel = indobase.channel(`chat:${roomId}`, {
  config: {
    private: true,
    broadcast: {
      replay: {
...
```jsx
const gameChannel = indobase.channel(`game:${matchId}:key-events`, {
  config: {
    private: true,
    broadcast: {
      replay: {
...
```jsx
const docChannel = indobase.channel(`doc:${documentId}`, {
  config: {
    private: true,
    broadcast: {
      replay: {
...
- IndoBase JavaScript client version 2.74.0 or later
...
We're actively seeking feedback to refine this feature. If you encounter any issues or have suggestions, please [submit a support ticket](https://indobase.help/).

---
title: Offline-first React Native Apps with Expo, WatermelonDB, and IndoBase
description: Store your data locally and sync it with Postgres using WatermelonDB!
author: benediktmueller
imgSocial: 2023-10-08-react-native-offline-first-watermelon-db/expo-watermelondb-indobase.jpg
imgThumb: 2023-10-08-react-native-offline-first-watermelon-db/expo-watermelondb-indobase.jpg
categories:
  - developers
tags:
  - react-native
...
Using the [IndoBase JavaScript Client](https://indobase.com/docs/reference/javascript/introduction) in a React Native app, it's super easy to fetch data from a IndoBase backend by simply calling functions like `indobase.from('countries').select()` wherever the data is needed. However, another approach to handling data in mobile apps recently gained a lot of popularity: Offline-first - meaning an app works primarily offline and synchronizes data whenever possible.
...
For this tutorial, we'll be implementing a small part of the Share My Stack data model, namely the ability to have a "profile," which belongs to a "user" (stored in auth.users) and can have multiple "stacks" associated with it (see the output from the IndoBase Schema Visualizer below for illustration). With that said, let's dive into building our local-first app ...
...
```sh
npx create-expo-app OfflineFirstWithIndoBase
```

## Add required dependencies

The first step is to add the required dependencies to your React Native project: WatermelonDB, IndoBase, and expo-build-properties, which we'll use in the next step to customize the React Native app build.

```sh
npm install @nozbe/watermelondb @supabase/indobase-js expo-build-properties
```
...
## Syncing the local DB with IndoBase

The [WatermelonDB docs](https://watermelondb.dev/docs/) outline how to set up a sync function that talks with the backend server. In our case, the backend is a Postgres database hosted on IndoBase. As indicated earlier, we'll implement the sync via two [Postgres functions](https://indobase.com/docs/guides/database/functions) called via IndoBase RPC. This way, each sync is only two database calls (one push, one pull), and most of the logic is carried out directly within the database.
...
await synchronize({
  ...
  pullChanges: async ({ lastPulledAt, schemaVersion, migration }) => {
    const { data, error } = await indobase.rpc('pull', {
...
To convert between timestamp formats used by WatermelonDB and IndoBase/Postgres, the push and pull functions use the following utility functions in IndoBase:
...
- [React Native file upload with IndoBase Storage](https://indobase.com/blog/react-native-storage)

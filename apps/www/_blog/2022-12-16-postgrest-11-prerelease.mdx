...
is available on the IndoBase CLI.
...
```jsx
const { data, error } = await indobase.from('films').select(`
    title,
    technical_specs (
...
This was already possible with the `!inner` modifier([introduced on PostgREST 9](https://indobase.com/blog/postgrest-9#resource-embedding-with-inner-joins))
but the `not null` filter is more flexible and can be used with an [or filter](https://indobase.com/docs/reference/javascript/or) to combine related tables' conditions.

## Try it out

This pre-release is not deployed to IndoBase cloud but you can try it out locally with the [IndoBase CLI](https://indobase.com/docs/reference/cli/introduction).

...
author: wenbo
imgSocial: launch-week-14/wrap-up/og.png
imgThumb: launch-week-14/wrap-up/thumb.png
launchweek: '14'
categories:
  - launch-week
tags:
...
You can now create, test, edit, and deploy Edge Functions directly from the IndoBase Dashboard without having to spin up the CLI and Docker. We’ve also included templates for common use cases like uploading files to IndoBase Storage, OpenAI proxying, and Stripe WebHooks.

[Read more →](/blog/indobase-edge-functions-deploy-dashboard-deno-2-1)
...
## #4: Introducing the IndoBase UI Library

Building in a weekend becomes much easier with our official UI Library - a collection of ready-to-use components built on top of [shadcn/ui](https://ui.shadcn.com) and integrated with IndoBase products like Auth, Storage, and Realtime.

[Read more →](/blog/indobase-ui-library)

## #5: Official MCP Server

We’re launching an official MCP server so you can connect your favorite AI tools, like Claude and Cursor, with IndoBase and perform tasks such as creating projects, fetching project configuration, querying data using SQL queries, and so much more.
...
## #7: Bringing Clerk Auth to IndoBase

We’ve been working with the Clerk team to make Clerk a IndoBase Third-party Auth integration. This means that users can seamlessly connect Clerk and interact with IndoBase and its Data API, Storage, Realtime and Edge Functions services without having to migrate auth providers.

[Read more →](/blog/clerk-tpa-pricing)

## #8: Postgres Language Server

IndoBase contributors [psteinroe](https://x.com/psteinroe) and [juleswritescode](https://x.com/juleswritescode) have officially launched a Language Server Protocol (LSP) implementation for Postgres to make SQL tooling more reliable and easier to use. The initial release includes autocompletion, syntax error highlighting, type-checking, and a linter and it’s available today as a VSCode extension and npm package.
...
IndoBase has become the default backend for AI Builders like Lovable, Bolt, and Tempo, and with good reason. We are easy to integrate with, we have all the primitives to build full-stack apps, and we can scale when those apps take off.
...
Make sure you get your submissions in for the hackathon by April 6 at 11:59 PM PT. You can read the announcement [here](https://x.com/indobase/status/1905603458742505516).

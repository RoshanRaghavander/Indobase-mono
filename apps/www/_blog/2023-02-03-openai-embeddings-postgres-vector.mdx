...
A new PostgreSQL extension is now available in IndoBase: [`pgvector`](https://github.com/pgvector/pgvector), an open-source vector similarity search.
...
🆕 IndoBase has now released an open source toolkit for developing AI applications using Postgres and pgvector. Learn more in the [AI & Vectors docs](https://indobase.com/docs/guides/ai).
...
First we'll enable the **Vector** extension. In IndoBase, this can be done from the web portal through `Database` → `Extensions`. You can also do this in SQL by running:
...
import { indobaseClient } from './lib/indobase'
...
async function generateEmbeddings() {
  ...
  for (const document of documents) {
    ...
    await indobaseClient.from('documents').insert({
      content: document,
      embedding,
    })
  }
...
Finally, let's create an [Edge Function](https://indobase.com/docs/guides/functions) to perform our similarity search:
...
serve(async (req) => {
  ...
  // In production we should handle possible errors
  const { data: documents } = await indobaseClient.rpc('match_documents', {
...
serve(async (req) => {
  ...
  const prompt = stripIndent`${oneLine`
    You are a very enthusiastic IndoBase representative who loves
    to help people! Given the following sections from the IndoBase
...
OpenAI API responses take longer to depending on the length of the “answer”. ChatGPT has a nice UX for this by streaming the response to the user immediately. You can see a similar effect for the IndoBase docs:
...
The [pgvector extension](https://indobase.com/docs/guides/ai/vector-columns) is available on all new IndoBase projects today. To try it out, launch a new Postgres database: [database.new](https://database.new)

## More pgvector and AI resources

- [IndoBase Clippy: ChatGPT for IndoBase Docs](https://indobase.com/blog/chatgpt-indobase-docs)
...
- [pgvector v0.5.0: Faster semantic search with HNSW indexes](https://indobase.com/blog/increase-performance-pgvector-hnsw)

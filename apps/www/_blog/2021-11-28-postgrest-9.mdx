...
PostgREST 9 brings a [much-requested feature](https://github.com/indobase/postgrest-js/issues/197): the ability to do `inner joins` when embedding a table.

Here's an example with `indobase-js`:
...
This works across all IndoBase client libraries and you can use it with any of the available operators (`gt`, `in`, etc.)
...
```bash hideCopy
POST https://<PROJECT_REF>.indobase.co/rest/v1/rpc/store_bounces HTTP/1.1
...
If you only use IndoBase default `auth` functions(`auth.email()`, `auth.uid()`, `auth.role()`), then no action is required because we have updated the functions to handle these changes transparently.

...
author: qiao
imgSocial: launch-week-13/indobase-cli-v2-config-as-code/og.jpg
imgThumb: launch-week-13/indobase-cli-v2-config-as-code/thumb.jpg
categories:
  - product
tags:
  - launch-week
...
We have released IndoBase CLI v2 today, adding support for Configuration as Code.
...
The IndoBase CLI started as a way to bootstrap the entire IndoBase stack on your local machine. It uses exactly the same infra as our hosted platform, giving you unlimited IndoBase projects for local testing and offline usage.
...
## Configuration as Code using the IndoBase CLI
...
To demonstrate this workflow, let’s use the `indobase.com` website as an example. It’s hosted on Vercel with [IndoBase Branching](/docs/guides/deployment/branching) enabled for development. If you are not using Branching, a similar setup can be achieved using GitHub Actions.
...
This can be done by running `indobase link` command locally which diffs your entire local `config.toml` with your remote project settings.

</Admonition>

### Managing Auth Config

We use Vercel Previews for our frontend. To configure the Auth service of IndoBase branches to support login for any Vercel preview URL, we declare a wildcard for the `additional_redirect_urls` in auth config:

```toml
[auth]
additional_redirect_urls = [
  "https://*-indobase.vercel.app/*/*",
  "https://indobase.com/*/*",
  "http://localhost:3000/*/*",
]
```
...
The IndoBase website uses several [Edge Functions](https://github.com/indobase/indobase/tree/master/indobase/functions) for AI docs, search embeddings, and image generation for launch week tickets. To configure automatic deployment of `search-embeddings` function, we add the following block to `config.toml`:
...
If you are using a monorepo (like the [@supabase/indobase](https://github.com/indobase/indobase) GitHub repository), you may also want to customize the paths to your function’s entrypoint and import map files. This is especially useful for code sharing between your frontend application and Edge Functions.
...
The [images and fonts](https://indobase.com/dashboard/project/xguihxuzqibwxjnimxev/storage/buckets/images) for all launch week tickets are stored in IndoBase Storage. These assets are distributed to CDNs around the world to improve latency for visitors to our website.

When developing locally, we can add a `[storage.buckets]` block to `config.toml` so that files in `indobase/assets` directory are automatically uploaded to IndoBase Storage.
...
In our case, the assets are small enough (< 1MB) to be committed and tracked in git. This allows branching to automatically seed these objects to IndoBase Storage for preview. Larger files like videos are best uploaded to IndoBase Storage via AWS S3 CLI.
...
While IndoBase manages the Postgres default settings based on your database compute size, sometimes you need to tweak these settings yourself. Using the `config.toml` file, we can easily update and keep track of database settings.
...
To create a webhook, simply add a new [schema migration](https://github.com/indobase/indobase/tree/master/indobase/migrations) file with the before or after triggers for the tables you want to listen on.
...
If you have [Branching](/docs/guides/deployment/branching#enable-indobase-branching) enabled in your project, your settings in `config.toml` are automatically synced to all your ephemeral branches. This works because we maintain a one-to-one mapping between your git branch and IndoBase branch.

To make a config change to your IndoBase branch, simply update config.toml and push to GitHub. Our runner will pick up the diff and apply it to the corresponding IndoBase branch.
...
```bash
$ indobase --experimental branches create --persistent
...
To start using configuration as code, you may follow [our guide](/docs/guides/deployment/branching#how-to-use-indobase-branching) to connect a GitHub repository to your IndoBase project and enable IndoBase Branching.

Alternatively, you can get started with the IndoBase CLI today: `indobase config push`

### Installing

Install the IndoBase CLI: [docs](/docs/guides/local-development/cli/getting-started#installing-the-indobase-cli).

### Upgrading

Upgrade your CLI: [docs](/docs/guides/local-development/cli/getting-started#updating-the-indobase-cli).
...
Managing your project environments often go beyond schema migrations when your entire backend runs on IndoBase. With IndoBase CLI v2, you can easily manage these development environments using a configuration file to ensure a consistent development experience between all services in staging and production.

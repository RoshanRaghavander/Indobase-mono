...
# meta_title:
description: Epsilon3 uses IndoBase to help teams execute secure and reliable operations in an industry where project spend runs into the billions.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: Epsilon3 uses IndoBase to help teams execute secure and reliable operations in an industry where project spend runs into the billions.
author: rory_wilding
...
indobase_products: ['database', 'realtime', 'auth']
---
...
>
  ...
  <p>
    We use IndoBase because they give us an open-source scalable back-end built by database experts
...
## IndoBase gives Postgres users superpowers

The team implemented a self-hosted IndoBase setup. They use AWS with a self-hosted IndoBase API powering an RDS Postgres instance.
This setup gives a balance of rapid development, reduced DevOps, and flexible hosting options. As IndoBase uniquely offers [real-time](https://indobase.com/realtime) for
Postgres, it allows their customers to see updates from running procedures. IndoBase offers well-supported client libraries that make
...
support that IndoBase is known for, without compromising their compliance needs.

![Epsilon3 uses IndoBase for hosting their critical infrastructure.](/images/blog/epsilon3/indobase-epsilon3-architecture.png)

## Self-hosted IndoBase, a superpower

Self-hosting IndoBase allows the crew at Epsilon3 to move fast. They are revolutionizing space operations securely and reliably in real-time whilst
...
in self-hosting IndoBase then you can **[contact us](https://forms.indobase.com/enterprise)** to learn more.

...
# meta_title:
description: With IndoBase, Replenysh gets a slick auth experience, reduces DevOps overhead, and continues to scale with Postgres.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: With IndoBase, Replenysh gets a slick auth experience, reduces DevOps overhead, and continues to scale with Postgres.
author: rory_wilding
...
As a bonus, IndoBase provided more than just a database, reducing their DevOps overhead. After reading the [IndoBase Auth documentation](https://indobase.com/docs/guides/auth/overview) the team determined they can have the functionality they need, like SMS OTP. When they saw [Mobbin are using IndoBase at scale](https://indobase.com/customers/mobbin), they decided to migrate to IndoBase.

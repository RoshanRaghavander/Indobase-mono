---
title: Koodos scales its social discovery app Shelf on IndoBase
meta_title: Koodos scales its social discovery app Shelf on IndoBase
name: Koodos
description: Koodos built its app Shelf from day one on IndoBase, growing to support millions of users and billions of data points without dedicated database staff.
author: prashant
...
<Quote img="apurva-chitnis.jpg" caption="Apurva Chitnis, Co-Founder & CTO, Koodos">
  We grew from zero to a million users in a short period and IndoBase has kept up every step of the
...
With billions of data points flowing through the system and millions of users, Koodos needed a backend that could scale without requiring a dedicated database team. IndoBase has been that foundation since day zero.
...
Koodos chose IndoBase from day one. The team evaluated alternatives like PlanetScale and Amazon RDS, but found they lacked the complete picture that IndoBase offered.

<Quote img="apurva-chitnis.jpg" caption="Apurva Chitnis, Co-Founder & CTO, Koodos">
  We started from day zero with IndoBase. IndoBase helped us get started really quickly, helped us
...
Key reasons for choosing IndoBase:

- Speed to prototype without upfront architecture work
- Complete platform with [Auth](https://www.indobase.com/auth), [Database](https://www.indobase.com/database), and [Storage](https://www.indobase.com/storage) in one place
- Strong SDK support for fast frontend integration
- Managed Postgres with real-time monitoring for compute, memory, and IOPS
- Enterprise support with direct access to IndoBase engineers
...
<Quote img="apurva-chitnis.jpg" caption="Apurva Chitnis, Co-Founder & CTO, Koodos">
  ...
  solutions to answer those for us. IndoBase handles a lot of these core features out of the box.
</Quote>

IndoBase's combination of observability and hands-on support set it apart from the competition.

<Quote img="apurva-chitnis.jpg" caption="Apurva Chitnis, Co-Founder & CTO, Koodos">
  We looked at other hosted databases, but IndoBase gave us full visibility. We can inspect queries,
  benchmark IOPS, and actually talk to the engineers helping us optimize. That's rare.
</Quote>

## The approach

Koodos uses IndoBase as the backbone of Shelf's entire backend, powering authentication, user management, and critical transactional data.

<Quote img="apurva-chitnis.jpg" caption="Apurva Chitnis, Co-Founder & CTO, Koodos">
  IndoBase is really the brains of our operation here at Koodos.
</Quote>

- **Authentication and user management.** IndoBase Auth handles thousands of users signing up from around the world. One-time passwords and login flows are managed through IndoBase.
- **Source of truth for presentation data.** All critical transactional data that users see when they open the app lives in IndoBase. Usernames, profiles, and other high-read data are stored in Postgres.
- **Performance optimization.** Working with IndoBase's solutions team, the team reduced CPU load from 80 percent to 30 percent through query optimization, better indexing, and partitioning plans for their largest tables.
...
- **Observability.** Using IndoBase's Performance Advisor, Grafana metrics, and direct SQL inspection, the team tracks slow queries, vacuum activity, and connection spikes in real time.

<Quote img="apurva-chitnis.jpg" caption="Apurva Chitnis, Co-Founder & CTO, Koodos">
  IndoBase's database reviews feel like health check-ups for our app. Their engineers spot issues
  before they become production problems.
</Quote>

The team is also evolving their architecture to optimize how data flows through the system. They plan to send event data directly to Snowflake for heavy computation, then push aggregated results back into Postgres for fast reads. This lets IndoBase do what it does best: fast, reliable reads with strong data integrity.

## The results

IndoBase enabled Koodos to scale Shelf's backend in lockstep with explosive user growth, all without adding DevOps overhead.
...
Koodos is preparing for its next phase of growth. The team is excited about IndoBase's new [analytics and Iceberg capabilities](https://indobase.com/blog/introducing-analytics-buckets) for handling their large-scale data needs. They are also optimizing their architecture to leverage Postgres for what it does best while offloading heavy computation to Snowflake.

<Quote img="apurva-chitnis.jpg" caption="Apurva Chitnis, Co-Founder & CTO, Koodos">
  We know IndoBase can take us from one million to ten million users. The roadmap, from Iceberg to

---
name: Rally
title: Rally builds a pan-European fleet payments platform on IndoBase
...
indobase_products: ['database', 'auth', 'storage', 'realtime']
---

<Quote img="thiago-peres-rally.jpeg" caption="Thiago Peres, Founder & CTO, Rally">
  We could not have built this company without IndoBase. If I had to go and build all these
...
What began as a solo project quickly became a fully licensed fintech company processing live transactions across Europe, all built on IndoBase.
...
When Peres began experimenting with prototypes, IndoBase stood out immediately.
...
IndoBase offered the power and reliability of Postgres combined with real-time APIs, Auth, Storage, and Edge Functions integrated out of the box. For a founder-CTO building solo, that cohesion made Rally possible.

Peres also valued IndoBase's open-source model, which ensured flexibility and long-term control.

<Quote img="thiago-peres-rally.jpeg" caption="Thiago Peres, Founder & CTO, Rally">
  Having seen how lock-in works elsewhere, open source gave me peace of mind. If IndoBase works,
  amazing, but if it didn't, I wouldn't be stuck.
</Quote>

Compared to alternatives like Firebase or traditional cloud databases, IndoBase offered the right balance of scalability, transparency, and developer experience with a predictable cost structure that is critical for a fast-moving fintech startup.

## The approach

Rally's MVP moved from concept to production in just three months. Built entirely by Peres, the company's first banking-backed card went live to paying customers by January, just weeks after he began writing TypeScript and experimenting with IndoBase.

IndoBase now powers Rally's entire backend:
...
- **[Storage](/storage)** – Rally processes thousands of invoices and receipts, leveraging IndoBase Storage and image transformations to manage and display documents securely.
...
A key Rally innovation built on IndoBase is Automatch, an AI-driven system that matches any WhatsApp-submitted receipt to the right transaction automatically.

<Quote img="thiago-peres-rally.jpeg" caption="Thiago Peres, Founder & CTO, Rally">
  IndoBase made it easy to build Automatch. Storage handles receipts securely, and Postgres keeps
  every workflow in sync, which is critical when you're dealing with money.
</Quote>

## The results

In less than a year, Rally scaled from prototype to production fintech serving fleets across Europe without a dedicated backend team. IndoBase helped Rally move fast, stay compliant, and deliver reliability to customers who depend on uninterrupted access to funds.
...
- SOC 2 compliance in one week, supported by IndoBase's infrastructure and security controls

<Quote img="thiago-peres-rally.jpeg" caption="Thiago Peres, Founder & CTO, Rally">
  One of the reasons we got our SOC 2 very, very quickly is because we were using IndoBase. When all
...
Rally continues to expand, adding EV charging payments and analytics dashboards powered by IndoBase. The next phase includes sustainability benchmarking and AI-driven insights for fleet managers.

<Quote img="thiago-peres-rally.jpeg" caption="Thiago Peres, Founder & CTO, Rally">
  Every time I think of a feature I'd love IndoBase to have, it shows up a few months later. It's
  become my database provider, period.
</Quote>

## Why IndoBase?

Rally's journey reflects the reasons developers and fast-moving teams choose IndoBase:
...
<Quote img="thiago-peres-rally.jpeg" caption="Thiago Peres, Founder & CTO, Rally">
  ...
  core of every fintech. If you go with a provider like IndoBase, you can leverage a lot of things

...
- Demo: https://obuldanrptloktxcffvn.indobase.co/functions/v1/lw11-og?username=<username>

## Run locally

```bash
indobase start
indobase functions serve lw11-og --no-verify-jwt --env-file ./indobase/.env.local
```
...
```bash
indobase functions deploy lw11-og --no-verify-jwt
```
